...
class KMKModel extends BaseModel {
    ...
    public function DtBankCompanyKI($param) {
        ...
        //          WHERE B.ACTIVATION = 'Y' 
        //       ORDER BY FCNAME ASC) GROUP BY FCNAME ORDER BY FCNAME ASC ";
        $SQL = "SELECT FCNAME, FCCODE FROM SUPPLIER WHERE SUPPLIER_TYPE = 'BANK' ORDER BY FCNAME ASC" ;
        $result = $this->db->query($SQL, $param['COMPANY'])->result();
        $this->db->close();
        return $result;
    }
    ...
    function GetDataWA($param){
        ...
        // if($get_where->SUB_CREDIT_TYPE == "BD"){
            $SQL = "SELECT L.*, L.UUID as LID, FM.PK_NUMBER AS REF_CONTRACT, BU.ID AS BUID, BU.FCNAME AS BUFCNAME,B.BANKACCOUNT || ' - ' || B.FCNAME || ' - ' || B.CURRENCY AS GETBANK, L.SUB_CREDIT_TYPE AS SUB_CREDIT_TYPEMASTER,FR.SUB_CREDIT_TYPE as SCT, FR.*, FR.ID AS ID_DETAIL FROM FUNDS_MASTER L LEFT JOIN (SELECT * FROM FUNDS_DETAIL_WA WHERE POS = '0' ) FR ON FR.UUID = L.UUID LEFT JOIN BUSINESSUNIT BU ON BU.ID = L.BUNIT LEFT JOIN BANK B ON B.FCCODE = L.BANK LEFT JOIN (SELECT UUID, PK_NUMBER FROM FUNDS_MASTER) FM ON FM.UUID = L.CONTRACT_REF WHERE L.UUID = ? --AND FR.POS = '0' 
            ORDER BY FR.POS ASC";
   ...
   FROM FUNDS_WD_KI FW
    ...
    public function ShowDataWithdraw($param){
        ...
        $q = "SELECT * FROM (SELECT FM.UUID,
                    ...
                    ON FDK.UUID = FM.UUID
            WHERE FM.IS_ACC = '1' AND FM.ISACTIVE = '1' AND FM.IS_COMPLETE IS NULL)
...
FETCH FIRST 1 ROWS ONLY) AS LIMIT_WA,
    ...
    public function getOWTrancheBalanceKI($param) {
        $q = "SELECT UUID, TRANCHE_NUMBER, LIMIT_TRANCHE, BALANCE, CURRENCY, BATCHID FROM
        (SELECT M.*, MAX(COUNTER) OVER (PARTITION BY UUID, TRANCHE_NUMBER) max_counter from
            (
            SELECT FKIT.UUID, FKIT.TRANCHE_NUMBER, FKIT.LIMIT_TRANCHE, WDKIT.IS_PAYMENT, FKIT.CURRENCY, WDKIT.BATCHID,
                    CASE
                       ...
                       (
                        SELECT
                          ...
                          FROM FUNDS_WD_KI_TRANCHE X
                               LEFT JOIN (
              SELECT MAX (BATCHID) AS BATCHID, TRANCHE_NUMBER, UUID
                FROM FUNDS_WD_KI_TRANCHE
               WHERE STATUS = 1
            GROUP BY TRANCHE_NUMBER, UUID) A
                                  ON A.TRANCHE_NUMBER = X.TRANCHE_NUMBER AND A.UUID = X.UUID AND A.BATCHID = X.BATCHID
                               LEFT JOIN (
               SELECT MAX(COALESCE(COUNTER, 0)) AS COUNTER , TRANCHE_NUMBER, UUID
                FROM FUNDS_WD_KI_TRANCHE
                WHERE STATUS = 1
                GROUP BY TRANCHE_NUMBER, UUID
                               ) B ON B.TRANCHE_NUMBER = X.TRANCHE_NUMBER AND B.UUID = X.UUID AND B.COUNTER = X.COUNTER
        ...
        ) WHERE COUNTER = MAX_COUNTER AND UUID = ?" ;
