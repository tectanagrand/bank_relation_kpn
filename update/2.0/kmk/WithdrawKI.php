...
<div class="panel panel-success">
    ...
    <div class="panel-heading">
        ...
        <h4 class="panel-title">Withdraw Kredit Investasi</h4>
    </div>
                    ...
                    <form id="FAddEditFormbd" data-parsley-validate="true" data-parsley-errors-messages-disabled="" onsubmit="return false" novalidate="">
                        ...
                        <div class="row">
                            <div class="form-group col-md-4">
                                ...
                                <select name="DRAWDOWN_TYPE" id="DRAWDOWN_TYPE" class="form-control">
                ...
                <div class="modal-content">
                    ...
                    <form id="FDetail" data-parsley-validate="true" data-parsley-errors-messages-disabled onsubmit="return false">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-primary" id="btnTranche" onclick="SaveWADetails(attachment)">Save</button>
    ...
    <?php if (!empty($_GET)) { ?>
                        <div class="panel-footer text-left">
                            ...
                            <button type="button" class="btn btn-success btn-sm m-l-5" onclick="Done()">Done</button>
                        </div>
                    <?php } ?>
</div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        // $('#IVENDOR').append(companyOpt).trigger('change');
        // $('#DRAWDOWN_TYPE').on('change', function() {
        //         if($('DRAWDOWN_TYPE').val() == 'COMBINE') {
                
        //         $('#IVENDOR').append($('<option>', {
        //                         value: companyCode,
        //                         text: companyName
        //                     }));
        //     }
                                    ...
                                    "dataSrc": function(ext) {
                                        if (ext.status == 200) {
                                            ...
                                            for (var index = 0 ; index < ext.result.data.length; index++) {
                                                ...
                                                var totdrw = totalDrawdown.get(trNum);
                                                var lastBalance = Number(totdrw) ;
                                            ...
                                            var tot_draw = totalDrawdown.get($('#TRANCHE_NUMBER option:selected').text());
                                            if(dum_bal) {
                                                $('#AMOUNT_BALANCE').val(fCurrency(String(dum_bal[0])));
                                            }
                                            $('#DRAWDOWN_VALUE').val(fCurrency(String(tot_draw)));
    ...
    $('#DRAWDOWN_VALUE').on('focus', function () {
        ...
        if(edited == 0 || check) {
            AMTMODALS = formatDesimal($("#DRAWDOWN_VALUE").val());
                                    ...
                                    if(response.status == 200) {
                                        ...
                                        if(!startWdByCurr) {
                                            tranches_number.map((x, idx) => {
                                                // console.log(x.TRANCHE_NUMBER,x.CURRENCY,idx);
                                                if(x.TRANCHE_NUMBER == fd.get("TRANCHE_NUMBER")) {
                                                    curr = x.CURRENCY ;
                                                }   
                                            });
                                            tranches_number.map((x, idx) => {
                                            ...
                                            var trnumber = fd.get("TRANCHE_NUMBER");
...
$(document).ready(function() {
    ...
    $('#IDRAWDOWN_VALUE').on('change keyup', function (){
        ...
        // console.log(bal, tnum);
        var ddown = Number(formatDesimal($(this).val()));
        if( ddown > Number(bal)) {
            toastr.error('Drawdown Value Over Limit') ;
            $('#btnTranche').attr('disabled', true);
        } 
        else {
            $('#btnTranche').attr('disabled', false);
        }
    }) ;
