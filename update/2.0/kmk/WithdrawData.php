...
<div class="panel panel-success">
    ...
    <div class="panel-heading">
        ...
        <h4 class="panel-title">Withdraw Data</h4>
    </div>
...
<script>
    ...
    $(document).ready(function () {
        if (getUrlParameter('type') == "edit" || getUrlParameter('type') == "add") {
            ...
            if (getUrlParameter('type') == "add") {
                ...
                var lastBalance = <?php echo json_encode($lastBalance); ?>; 
                // console.log(data);
                SetData(data);
                if((data.STATUS == '1' || data.STATUS == '0' || data.STATUS == '2') && getUrlParameter('vm') == "vm"){
                ...
                // if((data.STATUS == '1' || data.STATUS == '0') && getUrlParameter('vm') == null){
                //     $('#btnSaveMaster').hide();
                //     $('.panel-footer #btnSaveMaster').hide();
                //     // $('.fileupload-buttonbar').hide(); 
                // }
            ...
            if (!$.fn.DataTable.isDataTable('#DtLeasing')) {
                $('#DtLeasing').DataTable({
                    "processing": true,
                    "ajax": {
                        "url": "<?php echo site_url('Kmk/ShowWithdrawData') ?>",
                        dataType: "JSON",
                        type: "POST",
                    ...
                    {"data": "CREDIT_TYPE"},
                    {
                        ...
                        render: function (data, type, row, meta) {
                            var html;
                            if (data.SUB_SCT_1 != null) {
                                html = data.SUB_SCT_1;
                            }
                            else if (data.SUB_CREDIT_TYPE_1 != null) {
                ...
                table.on('click', '.edit', function() {
                    ...
                    if(data.SUB_CREDIT_TYPE == 'TL'){
                        window.open("<?php echo site_url('WithdrawDataTL'); ?>" + '?type=edit&UUID=' + data.UUID + '&IDDET='+data.ID,'_self');
                    }
                    if(data.SUB_CREDIT_TYPE == 'WA' && data.SUB_CREDIT_TYPE_1 == 'WA'){
                        window.open("<?php echo site_url('WithdrawDataWA'); ?>" + '?type=edit&UUID=' + data.UUID + '&IDDET='+data.ID,'_self');   
                    }
                    if(data.SUB_CREDIT_TYPE == 'WA' && data.SUB_SCT_1 == 'KMK_SCF_AR'){
                        window.open("<?php echo site_url('WithdrawDataAR'); ?>" + '?type=edit&UUID=' + data.UUID + '&IDDET='+data.ID + '&IDDETAP='+data.IDDETAPAR,'_self');   
                    }
                    if(data.SUB_CREDIT_TYPE == 'WA' && data.SUB_SCT_1 == 'KMK_SCF_AP'){
                        window.open("<?php echo site_url('WithdrawDataAP'); ?>" + '?type=edit&UUID=' + data.UUID + '&IDDET='+data.ID + '&IDDETAP='+data.IDDETAPAR,'_self');   
                    }
                    if(data.SUB_CREDIT_TYPE == 'FINANCING' || data.SUB_CREDIT_TYPE == 'REFINANCING'){
                        window.open("<?php echo site_url('WithdrawDataKI'); ?>" + '?type=edit&UUID=' + data.UUID,'_self');   
                    }
                    if(data.SUB_CREDIT_TYPE == 'BD'){
                        window.open("<?php echo site_url('WithdrawDataNDK'); ?>" + '?type=edit&UUID=' + data.UUID + '&IDDET='+ data.ID,'_self');    
                    }
                    // else{
    ...
    var CREDIT_TYPE,SUB_CREDIT_TYPE,IDDET;
    ...
    function SetData(data) {
        ...
        if(data.AMOUNT_BALANCE == null || data.AMOUNT_BALANCE == 0){
            $('#AMOUNT_BALANCE').val(fCurrency(lastBalance.AMOUNT_BALANCE ? lastBalance.AMOUNT_BALANCE : data.AMOUNT_LIMIT ));    
        }else{
    ...
    var SaveWD = function () {
        ...
        if ($('#FAddEditFormbd').parsley().validate()) {
                ...
                $.ajax({
                    dataType: "JSON",
                    type: "POST",
                    url: "<?php echo site_url('Kmk/SaveWDbyId'); ?>",
                    data: {
                        UUID: ID,
                        IDDET:IDDET,
                    ...
                    success: function (response) {
                        ...
                        if (response.status == 200) {
                            ...
                            setTimeout(function() { window.location.href = "<?php echo site_url('WithdrawData'); ?>"; },1000);
