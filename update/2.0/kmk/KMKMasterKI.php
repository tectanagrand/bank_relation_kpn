            ...
            <div class="panel panel-default">
                ...
                <!-- - -->
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-3">
                            <label for="company">Payment Bank Account *</label>
                            <input class="form-control" type="text" name="PAYMENT_BANK_ACCOUNT" id="PAYMENT_BANK_ACCOUNT" placeholder="Payment Bank Account">
                        </div>
                    </div>
                    <div class="row">
                        <div class="form-group col-md-6">
...
<script>
    ...
    function SetData(data, bankKI) {
        ...
        $('#PAYMENT_BANK_ACCOUNT').val(data.PAYMENT_BANK_ACC);
        $('#UUID').val(UUID);
        ...
        if ($('#FAddEditFormbd').parsley().validate() && (($('#CTYPE').val() == 'SYNDICATION') ? ($('#BANK_SYND').parsley().validate()) : 1)) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        PAYMENT_BANK_ACCOUNT : $('#PAYMENT_BANK_ACCOUNT').val(),
                        POS: $('#pos').val(),
                        CTYPE : $('#CTYPE').val(),
                        ACTION: ACTION,
                        USERNAME: USERNAME,
    ...
    if (!$.fn.DataTable.isDataTable('#DtDetail')) {
                            ...
                            // {"className": "text-center","data":"BANK_PORTION"},
                            {
                                        ...
                                        render: function (data, type, row, meta) {
                                            ...
                                            if (EDITS == 1) {
                                                ...
                                                if(row.ISACTIVE == 1) {
                                                    html += '<button class="btn btn-danger btn-icon btn-circle btn-sm disableTr" title="Disable" style="margin-right: 5px;">\n\
                                                    <i class="fas fa-times-circle" aria-hidden="true"></i>\n\
                                                    </button>';
                                                } 
                                                else {
                                                    html += '<button class="btn btn-success btn-icon btn-circle btn-sm disableTr" title="Enable" style="margin-right: 5px;">\n\
                        ...
                        tbHeaderDet.on('click', '.disableTr', function() {
                            $tr = $(this).closest('tr');
                            var data = tbHeaderDet.row($tr).data();
                            if(confirm('Are you sure want to disable/enable this data ?')) {
                                $.ajax({
                                    dataType : "JSON",
                                    type : "POST",
                                    url : "<?php echo site_url('Kmk/DisableEnableTranche') ;?>",
                                    data: {
                                        ID: data.ID,
                                        USERNAME: USERNAME
                                    },
