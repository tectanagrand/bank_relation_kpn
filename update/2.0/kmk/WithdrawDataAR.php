...
<script>
    ...
    $(document).ready(function () {
        ...
        if (getUrlParameter('type') == "edit" || getUrlParameter('type') == "add" || getUrlParameter('type') == 'view' ) {
    ...
    function SetData(data) {
        ...
        $('#EXC_RATE').val(fCurrency(String(data.RATE)));
        $('#INTEREST').val(Number(data.INTEREST));
        $('#PROVISION').val(Number(data.PROVISION));
        $('#DATA_EXIST').val(Number(data.DATA_EXIST));
        //take balance
        $.ajax({
            dataType : "JSON",
            type : "POST",
        ...
        if ($('#FSCFAR').parsley().validate()) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        VALUE_DATE: $('#VALUE_DATE').val(),
                        BATCHID : BEGIN ? DATASCFAR.BATCHID : BATCHID,
            ...
            "columns": [
                ...
                {"className" : "text-center", "data":"FILE_NAME"},  
                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        html += '<button id="editbtn" class="btn btn-success btn-icon btn-circle btn-sm edit" title="edit" style="margin-right: 5px;">\n\
                        <i class="fas fa-edit" aria-hidden="true"></i>\n\
                        </button>';
                        }
                        if (DELETES == 1 && !((getUrlParameter('vm') == 'vm' || DtKmk.STATUS == 1 ||  DtKmk.STATUS == 2))) {
                            html += '<button id="deletebtn" class="btn btn-danger btn-icon btn-circle btn-sm delete" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i></button>';
                        }
                        html += '<button class="btn btn-success btn-icon btn-circle btn-sm dwn" title="Download" style="margin-right: 5px;">\n\
                        <i class="fas fa-arrow-down" aria-hidden="true"></i>\n\
                        </button>';
                        ...
                        success: function (response) {
                            if (response.status == 200) {
                                ...
                                $.ajax({
                                    dataType : 'JSON',
                                    type : 'POST',
                                    url : '<?php echo site_url('Kmk/GetDataWDAPARbyID'); ?>',
                                    data : {
                                        UUID    : DtKmk.UUID,
                                        IDDETAP      : urlParams.get('IDDETAP'),
                                        WD_TYPE : 'KMK_SCF_AR'
                                    },
                                    success : function(response) {
