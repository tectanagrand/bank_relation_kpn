...
class CronAPController extends CI_Controller {
    ...
    public function getDatass() {
        ...
        try {
            ...
            $PERIODFROM    = Date('Y-m-d', strtotime('-3 days')).'T'.'00:00:00';
    ...
    public function getDataManual() {
        ...
        try {
            ...
            for($i = 1;$i<=12;$i++){
                ...
                // $result = $this->DlapModel->DataUpload($dataJson['d']['results']);
                $result = $this->DataInputManual($dataJson['d']['results']);
            }
    ...
    public function DataInput($param){
        try {
            ...
            $this->db->delete('TEMP_UPLOAD_PO_FOR_TODAY');
            ...
            //                        array_push($data, $dt);
                $data[] = $dt;
                $this->db->flush_cache();
                $result = $this->db->insert('TEMP_UPLOAD_PO_FOR_TODAY', $dt);
                $idx++;
            //                        $result = $this->db->insert('TEMP_UPLOAD_PO_FOR_TODAY', $dt);
            }
            // echo "<pre>";
            // var_dump($data);exit;
            $this->db->flush_cache();
            // $result = $this->db->insert_batch(TEMP_UPLOAD_PO_FOR_TODAY, (array)$data);
            // end foreach
            // $this->db->flush_cache();
            if ($result) {
                // echo "<pre>";
    ...
    //Update Error Di Header PO
                $SQL = " UPDATE TEMP_UPLOAD_PO_FOR_TODAY UPO SET (STATUSH, MESSAGEH) = (SELECT DECODE(TP.JML, 1, (SELECT DISTINCT TEU.STATUSD
                                                        FROM TEMP_UPLOAD_PO_FOR_TODAY TEU
                                                        ...
                                                        AND TEU.DOCREF = TP.DOCREF ), 1) AS STATUSH, 
                                    DECODE(TP.JML, 1, (SELECT DISTINCT TEU.MESSAGED
                                                        ...
                                                        AND (TEU.BUSINESSUNITCODE = TP.BUSINESSUNITCODE OR TEU.BUSINESSUNITCODE IS NULL)
                                                        AND (TEU.DEPARTMENTCODE = TP.DEPARTMENTCODE OR TEU.DEPARTMENTCODE IS NULL)
                                                        AND TEU.DOCNUMBER = TP.DOCNUMBER AND TEU.DOCREF = TP.DOCREF), 'Please, Check Data Doc Date until Payterm must be the same, and there are no errors lined up !!') AS MESSAGEH
                            FROM (SELECT TUP.UUID, TUP.COMPANYCODE, TUP.BUSINESSUNITCODE, TUP.DEPARTMENTCODE, TUP.DOCNUMBER,TUP.DOCREF,COUNT(*) AS JML
                                    FROM (SELECT DISTINCT UUID, COMPANYCODE, BUSINESSUNITCODE, DEPARTMENTCODE, DOCNUMBER, DOCDATE, VENDOR, DOCTYPE, DOCREF, TRANS_LOC,
                                            ...
                                            FROM TEMP_UPLOAD_PO_FOR_TODAY
                                            WHERE UUID = ?) TUP
                                    GROUP BY TUP.UUID, TUP.COMPANYCODE, TUP.BUSINESSUNITCODE, TUP.DEPARTMENTCODE, TUP.DOCNUMBER,TUP.DOCREF) TP 
                            WHERE (TP.COMPANYCODE = UPO.COMPANYCODE OR TP.COMPANYCODE IS NULL)
                                ...
                                AND TP.DOCNUMBER = UPO.DOCNUMBER AND UPO.DOCREF = TP.DOCREF
                                AND TP.UUID = UPO.UUID) WHERE UPO.UUID = ?";
                $result = $this->db->query($SQL, [$UUID, $UUID]);
                
    //                  Select Header PO and Get Detail PO
                $SQL = "SELECT DISTINCT * FROM TEMP_UPLOAD_PO_FOR_TODAY TUP WHERE TUP.UUID = ?";
                ...
                //         FROM TEMP_UPLOAD_PO_FOR_TODAY TUP
                ...
                $SQL = "SELECT * FROM TEMP_UPLOAD_PO_FOR_TODAY 
                            WHERE UUID = ?
                ...
                // $SQL = "DELETE FROM TEMP_UPLOAD_PO_FOR_TODAY WHERE UUID = ?";
            ...
            foreach ($data as $value) {
                ...
                if($checkSubgroup->COMPANY_SUBGROUP == "DOWNSTREAM"){
                    $sub = "-DWS";
                }elseif($checkSubgroup->COMPANY_SUBGROUP == "UPSTREAM"){
                    $sub = "-UPS";
                }else{
                    $sub = '';
                }
                ...
                if($value['STATUSH'] != "1" || $value['STATUSH'] != 1){
                    ...
                    $dt = [
                        ...
                        'DEPARTMENT' => $value['DEPARTMENT'].$sub,
    ...
    public function DataInputManual($param){
        try {
            $this->db->trans_begin();
            $result = FALSE;
            $data = [];
            ...
            $idx        = 0;
            $this->db->where('CREATED_BY','NEWAPI_AP');
            $this->db->delete('TEMP_UPLOAD_PO_FOR_MANUAL');
            ...
            //                        array_push($data, $dt);
                $data[] = $dt;
                // $this->db->flush_cache();
                $result = $this->db->insert('TEMP_UPLOAD_PO_FOR_MANUAL', $dt);
                $this->db->flush_cache();
                $idx++;
            //                        $result = $this->db->insert('TEMP_UPLOAD_PO_FOR_MANUAL', $dt);
            }
            ...
            // var_dump($data);exit;
            $this->db->flush_cache();
            // $result = $this->db->insert_batch(TEMP_UPLOAD_PO_FOR_MANUAL, (array)$data);
            // end foreach
            // $this->db->flush_cache();
            if ($result) {
                // echo "<pre>";
                // var_dump(is_null($data[0]['AP_NO']));exit;
                $this->CheckValidation_MANUAL($EXTSYSTEM, $UUID, $USERNAME);
                foreach($data as $d => $rows){
                    // is_null($rows->AP_NO);exit;
                    if (is_null($rows['AP_NO']) === false) {
                        $this->CheckValidationAP_NO_MANUAL($EXTSYSTEM, $UUID, $USERNAME);
                    }
                    // $this->CheckValidationAP_NO($EXTSYSTEM, $UUID, $USERNAME);  
                }
                
    //Update Error Di Header PO
                $SQL = " UPDATE TEMP_UPLOAD_PO_FOR_MANUAL UPO SET (STATUSH, MESSAGEH) = (SELECT DECODE(TP.JML, 1, (SELECT DISTINCT TEU.STATUSD
                                                        FROM TEMP_UPLOAD_PO_FOR_MANUAL TEU
                                                        WHERE TEU.UUID = TP.UUID 
                                                        AND (TEU.COMPANYCODE = TP.COMPANYCODE OR TEU.COMPANYCODE IS NULL)
                                                        AND (TEU.BUSINESSUNITCODE = TP.BUSINESSUNITCODE OR TEU.BUSINESSUNITCODE IS NULL)
                                                        AND (TEU.DEPARTMENTCODE = TP.DEPARTMENTCODE OR TEU.DEPARTMENTCODE IS NULL)
                            ...
                            FROM (SELECT TUP.UUID, TUP.COMPANYCODE, TUP.BUSINESSUNITCODE, TUP.DEPARTMENTCODE, TUP.DOCNUMBER,TUP.DOCREF, COUNT(*) AS JML
                                    FROM (SELECT DISTINCT UUID, COMPANYCODE, BUSINESSUNITCODE, DEPARTMENTCODE, DOCNUMBER, DOCDATE, VENDOR, DOCTYPE, DOCREF, TRANS_LOC,
                                                    BASELINEDATE, PAYTERM, MESSAGED
                                            FROM TEMP_UPLOAD_PO_FOR_MANUAL
                                            WHERE UUID = ?) TUP
                                    GROUP BY TUP.UUID, TUP.COMPANYCODE, TUP.BUSINESSUNITCODE, TUP.DEPARTMENTCODE, TUP.DOCNUMBER,TUP.DOCREF) TP 
                            WHERE (TP.COMPANYCODE = UPO.COMPANYCODE OR TP.COMPANYCODE IS NULL)
                                AND (TP.BUSINESSUNITCODE = UPO.BUSINESSUNITCODE OR TP.BUSINESSUNITCODE IS NULL)
    ...
    //                  Select Header PO and Get Detail PO
                $SQL = "SELECT DISTINCT * FROM TEMP_UPLOAD_PO_FOR_MANUAL TUP WHERE TUP.UUID = ?";
                $data = $this->db->query($SQL, [$UUID])->result();
                // $SQL = "SELECT DISTINCT TUP.COMPANY, TUP.COMPANYCODE, TUP.BUSINESSUNIT, TUP.BUSINESSUNITCODE, TUP.DEPARTMENT, TUP.DEPARTMENTCODE, TUP.DOCNUMBER, TUP.DOCREF,TUP.STATUSH, TUP.MESSAGEH, TP.JML, TP.AMOUNT_INCLUDE_VAT, TP.AMOUNT_PPH
                //         FROM TEMP_UPLOAD_PO_FOR_MANUAL TUP
                //         INNER JOIN (SELECT UUID, COMPANYCODE, BUSINESSUNITCODE, DEPARTMENTCODE, DOCNUMBER,DOCREF, COUNT(*) AS JML, SUM(AMOUNT_INCLUDE_VAT) AS AMOUNT_INCLUDE_VAT, SUM(AMOUNT_PPH) AS AMOUNT_PPH
                //                     FROM TEMP_UPLOAD_PO_FOR_MANUAL 
                //                     WHERE UUID = ?
                //                     GROUP BY UUID, COMPANYCODE, BUSINESSUNITCODE, DEPARTMENTCODE, DOCNUMBER,DOCREF) TP
                ...
                //         WHERE TUP.UUID = ?";
                // $data = $this->db->query($SQL, [$UUID, $UUID])->result();
                $SQL = "SELECT * FROM TEMP_UPLOAD_PO_FOR_MANUAL 
                            WHERE UUID = ?
                            AND (COMPANYCODE = ? OR COMPANYCODE IS NULL)
                            AND (BUSINESSUNITCODE = ? OR BUSINESSUNITCODE IS NULL)
                            AND (DEPARTMENTCODE = ? OR DEPARTMENTCODE IS NULL)
                ...
                // $SQL = "DELETE FROM TEMP_UPLOAD_PO_FOR_MANUAL WHERE UUID = ?";
                // $result = $this->db->query($SQL, [$UUID]);
            ...
            $data = json_encode($data, TRUE);
            $data = json_decode($data, TRUE);
            $UPLOAD_REF = date("Ymd_His") . "_NEWAPI_AP_MANUAL";
            foreach ($data as $value) {
                $ADENDUM = 'FALSE';
                $checkSubgroup = $this->db->get_where('COMPANY',['ID' => $value['COMPANY']])->row();
                $sub = "";
...
//        Update Field Company and Pesan Error
        $SQL = "UPDATE TEMP_UPLOAD_PO_FOR_TODAY TUP
...
//        Update Field Vendor and Pesan Error
        $SQL = "UPDATE TEMP_UPLOAD_PO_FOR_TODAY TP 
...
//        Check Doc Ref Ready For INV or INV_AR
        $SQL = "UPDATE TEMP_UPLOAD_PO_FOR_TODAY UPO
...
//        Cek Docnumber Same
    ...
    public function CheckValidation_MANUAL($EXTSYSTEM, $UUID, $USERNAME) {
//        Update Field Company and Pesan Error
        $SQL = "UPDATE TEMP_UPLOAD_PO_FOR_MANUAL TUP
                   SET TUP.COMPANY = (SELECT c.ID
                                        FROM COMPANY C 
                                       INNER JOIN COMPANY_EXTSYS CE
                                               ON CE.COMPANY = C.ID
...
//        Update Field Vendor and Pesan Error
        $SQL = "UPDATE TEMP_UPLOAD_PO_FOR_MANUAL TP 
                   SET TP.VENDORCODE = (SELECT TUP.VENDORCODE
                                          FROM (SELECT TUP.COMPANYCODE, TUP.DOCNUMBER, MAX(TUP.VENDORCODE) AS VENDORCODE, COUNT(*) AS JML
                                                  FROM (SELECT DISTINCT TUP.COMPANYCODE, TUP.DOCNUMBER, TUP.VENDORCODE
                                                          FROM TEMP_UPLOAD_PO_FOR_MANUAL TUP
...
//        Check Doc Ref Ready For INV or INV_AR
        $SQL = "UPDATE TEMP_UPLOAD_PO_FOR_MANUAL UPO
                   SET (UPO.STATUSD, UPO.MESSAGED) = (SELECT DECODE(CT.JML, 0, 1, 0) AS STATUSD, DECODE(CT.JML, 0, 'Doc Ref Not Found !!', '') AS MESSAGED
                                                        FROM (SELECT COUNT(*) AS JML
                                                                FROM CF_TRANSACTION CTR
                                                               WHERE CTR.DOCNUMBER = UPO.DOCREF
...
//        Cek Docnumber Same
    ...
    public function CheckValidationAP_NO_MANUAL($EXTSYSTEM, $UUID, $USERNAME) {
        $SQL = "UPDATE TEMP_UPLOAD_PO_FOR_MANUAL TUP 
                   SET TUP.STATUSD = 1,
                       TUP.MESSAGED = 'Some Data Already Exists 2 !!!'
                 WHERE TUP.STATUSD <> 1
