...
class PaymentModel extends BaseModel {
    ...
    public function DtOsPayment1($param) {
        ...
        $whereNodoc = '';
        if($NODOC != null || $NODOC != ''){
            $whereNodoc .= " AND (CFT.DOCNUMBER = '$NODOC' OR CFT.DOCREF = '$NODOC' ) ";
        }
        ...
        $SQL = "SELECT DISTINCT CFT.*,
                    ...
                    FROM (SELECT CFT.*,
                            ...
                            FROM (SELECT CFT.ID,
                                    ...
                                    FROM CF_TRANSACTION CFT
                                         INNER JOIN CF_TRANSACTION CFPO
                                            ON     CFPO.DOCNUMBER = CFT.DOCREF
                                               AND CFPO.COMPANY = CFT.COMPANY AND CFPO.VENDOR = CFT.VENDOR
                                   WHERE     (CFT.DOCTYPE = 'INV' OR CFT.DOCTYPE = 'INV_AR' OR CFT.DOCTYPE = 'LEASING' OR CFT.DOCTYPE = 'LOAN' OR CFT.DOCTYPE = 'KI')
                                  ...
                                  SELECT CFT.ID,
                                         ...
                                         CFT.INVOICEVENDORNO
                                    FROM CF_TRANSACTION CFT
                                   WHERE     CFT.DOCTYPE IN ('PDO', 'INV_AP_SPC','INV_AR_SPC','LEASING','LOAN', 'KI')
                         ...
                         LEFT JOIN (SELECT DOCNUMBER FROM LEASINGMASTER WHERE ISACTIVE = 'TRUE') LM ON LM.DOCNUMBER = CFT.DOCNUMBER
                         LEFT JOIN
                   ...
                   WHERE     CFT.DATABY LIKE '$DATABY'
                         ...
                         AND CFT.AMOUNTOS > 0
                         AND TO_CHAR(CFT.FILTERDATE,'YYYY/MM') BETWEEN '$DATEFROM' AND '$DATETO' ".$whereNodoc." ORDER BY C.COMPANYNAME, CFT.DOCREF";
        // $ParamW = [
    ...
    public function SavePayment($Data, $Location) {
        ...
        $date  = strtotime($Data['DATERELEASE']);
        $cTime = date("mY",$date);
...
//            Cek Amount Bank
            ...
            foreach ($Data["DtPaid"] as $values) {
                ...
                if($values['DOCTYPE'] === "LEASING"){
                    $del = "DELETE FROM TASK_SCHEDULER WHERE DOCNUMBER = '".$values['DOCNUMBER']."' AND TO_CHAR(EMAILDATE,'mmyyyy') = '$cTime' AND TIPE = '1'";
                    $this->db->query($del);
                }
...
//            $SQLO = " ORDER BY FC.DATERELEASE, FC.VOUCHERNO, FC.DEPARTMENT, FC.COMPANYNAME, FC.DOCNUMBER";
    ...
    public function getCompanyClosing($param) {
        $Lenght = $param["length"];
        $Start = $param["start"];
        $Columns = $param["columns"];
        $Search = $param["search"];
        ...
        $OrderField = $Columns[$Order[0]["column"]]["data"];
        $WHERE = '';
        if($param['SUBGROUP'] != null || $param['SUBGROUP'] != ''){
            $WHERE .= " WHERE C.COMPANY_SUBGROUP = '".$param['SUBGROUP']."'";
        }
        $SQL = "(SELECT C.ID, C.COMPANYCODE || ' - ' || C.COMPANYNAME AS COMPANYNAME, PC.CURRENTACCOUNTINGYEAR,PC.CURRENTACCOUNTINGPERIOD,PC.CURRENTDATE,PC.CLOSEACCOUNTINGPERIOD,PC.CLOSEACCOUNTINGYEAR
                      FROM COMPANY C INNER JOIN PAYMENT_PERIODCONTROL PC ON PC.COMPANY = C.ID
                     ".$WHERE;
        

        $SQL .= "  ORDER BY C.COMPANYCODE)";
        $idx = 1;
        $SQLW = "";
        if ($Search["regex"] == 'true') {
            $Search['value'] = strtoupper($Search['value']);
...
//            $SQLO = " ORDER BY FC.DATERELEASE, FC.VOUCHERNO, FC.DEPARTMENT, FC.COMPANYNAME, FC.DOCNUMBER";
        ...
        $result = $this->db->query("SELECT * FROM $SQL FC $SQLW $SQLO OFFSET $Start ROWS FETCH NEXT $Lenght ROWS ONLY")->result();
        // var_dump($this->db->last_query());exit;
        $CountFil = $this->db->query("SELECT COUNT(*) AS JML FROM $SQL FC $SQLW")->result();
        $CountAll = $this->db->query("SELECT COUNT(*) AS JML FROM $SQL FC")->result();
        $return = [
            "data" => $result,
            "recordsTotal" => $CountAll[0]->JML,
            "recordsFiltered" => $CountFil[0]->JML
    ...
    public function ShowDataPaid($param) {
        ...
        $SQL = "(SELECT PY.PAYMENTID, PY.FORECASTID, PY.CFTRANSID, TO_CHAR(PY.DATERELEASE, 'MM/DD/YYYY') AS DATERELEASE, PY.BANKCODE, PY.VOUCHERNO, PY.NOCEKGIRO, PY.CASHFLOWTYPE, PY.REMARK, PY.AMOUNT AS AMOUNTPAY,
                       CFT.DOCNUMBER, CFT.DOCREF,CFT.INVOICEVENDORNO, CFT.AMOUNTINVOICE, CFT.COMPANY, CFT.COMPANYCODE, CFT.COMPANYNAME, CFT.BUSINESSUNIT, CFT.BUSINESSUNITCODE, CFT.BUSINESSUNITNAME, 
                       CFT.VENDOR, CFT.VENDORNAME, CFT.DEPARTMENT, B.FCNAME || ' - ' || B.BANKACCOUNT AS BANKNAME, B.BANKACCOUNT
                  FROM $this->PAYMENT PY
                 INNER JOIN (SELECT CFT.*, C.COMPANYCODE, C.COMPANYNAME, BS.FCCODE AS BUSINESSUNITCODE, BS.FCNAME AS BUSINESSUNITNAME, S.FCNAME AS VENDORNAME
                               FROM (SELECT CFT.ID, CFT.COMPANY, CFT.BUSINESSUNIT, CFPO.DEPARTMENT, CFT.DOCNUMBER, CFT.DOCREF, CFT.VENDOR,
                                            TO_CHAR(CFT.DUEDATE, 'YYYY/MM/DD') AS DUEDATE, (CFT.AMOUNT_INCLUDE_VAT - NVL(CFT.AMOUNT_PPH, 0)) AS AMOUNTINVOICE, 
                                     ...
                                     SELECT CFT.ID, CFT.COMPANY, CFT.BUSINESSUNIT, CFT.DEPARTMENT, CFT.DOCREF, CFT.DOCNUMBER, CFT.VENDOR, 
                                            ...
                                            CFT.DOCTYPE, TO_CHAR(CFT.DOCDATE, 'YYYY/MM/DD') AS DOCDATE,CFT.INVOICEVENDORNO
                                       FROM $this->CF_TRANSACTION CFT
                                      WHERE CFT.DOCTYPE in('PDO','INV_AP_SPC','LEASING','LOAN')) CFT 
                              INNER JOIN $this->COMPANY C 
                                      ON C.ID = CFT.COMPANY
                              INNER JOIN $this->BUSINESSUNIT BS
                                      ON BS.ID = CFT.BUSINESSUNIT
        ...
        if ($param["COMPANY"] != "" && $param["COMPANY"] != NULL && $param["COMPANY"] != "0") {
            $SQL .= " AND CFT.COMPANY = ?";
            array_push($ParamW, $param["COMPANY"]);
        }
        if ($param["VENDOR"] != "" && $param["VENDOR"] != NULL) {
            $SQL .= " AND CFT.VENDOR = ?";
            array_push($ParamW, $param["VENDOR"]);
        }
        if ($param["CASHFLOWTYPE"] != "" && $param["CASHFLOWTYPE"] != NULL) {
            $SQL .= " AND PY.CASHFLOWTYPE = ?";
            array_push($ParamW, $param["CASHFLOWTYPE"]);
...
//            Cek Amount Bank
            ...
            if($this->db->trans_status() === FALSE || !isset($result)){
               $this->db->trans_rollback();
               throw new Exception('Data Save Failed !!');
               $this->db->trans_rollback();
            }else{
...
//            Cek Period
    ...
    public function SearchOtherVoucher($param) {
        ...
        $SQL = "SELECT PO.PAYMENTID, PO.BANKCODE, PO.VOUCHERNO, PO.NOCEKGIRO,
                                    ...
                                    BK.FCNAME AS BANKNAME, BK.BANKACCOUNT,  BK.CURRENCY, PO.EXTSYS, MATERIAL.FCCODE MATERIALCODE, MATERIAL.FCNAME MATERIALNAME, PO.MATERIAL AS POMATERIALID,C.COMPANYCODE
                                    FROM
                                    PAYMENT_OTHER PO
    ...
    public function SaveOtherPayment($Data, $Location) {
        ...
        if($Data['VOUCHERNO'] == null || $Data['VOUCHERNO'] == ''){
            throw new Exception("Voucher Cant Null");
    ...
    public function ShowDataPeriod() {
        $this->fillable = ["C.COMPANYCODE || ' - ' || C.COMPANYNAME AS COMPANYNAME","P.CURRENTDATE","P.CURRENTACCOUNTINGYEAR" ,'P.CURRENTACCOUNTINGPERIOD', 'P.CLOSEACCOUNTINGYEAR', 'P.CLOSEACCOUNTINGPERIOD'];
    ...
    public function saveUpdatePeriod($param,$location){
        try {
            ...
            if ($result) {
                $log = "INSERT INTO PAYMENT_PERIODCONTROLLOG SELECT * FROM PAYMENT_PERIODCONTROL";
                $this->db->query($log);
                $this->db->trans_commit();
                $return = [
                    'STATUS' => TRUE,
        ...
        try {
            ...
            $q = "SELECT 
                ...
                FROM 
                  ...
                  ) where period_year = '$getPeriod->CURRENTACCOUNTINGYEAR' and period_month = '$getPeriod->CURRENTACCOUNTINGPERIOD' ";
            ...
            if ($result1) {
                ...
                foreach($result1 as $row){
                    ...
                    $where = array(
                        ...
                        'PERIOD_YEAR'  => $getValPeriod->PERIOD_YEAR,
                        'PERIOD_MONTH' => $getValPeriod->PERIOD_MONTH,
                        'COMPANY'      => $COMPANY
                    );
                    $this->db->where($where);
                    $upcurrent = $this->db->update('BANKBALANCE',$update);
                    $this->db->trans_commit();
                    $this->db->flush_cache();
                    ...
                    // var_dump($YEAR_NEXT.'-'.$MONTH_NEXT.'-'.$BANK_CODE);exit;
                    $qcekDupe = "SELECT * FROM BANKBALANCE WHERE COMPANY = '$COMPANY' AND PERIOD_YEAR = '$YEAR_NEXT' AND PERIOD_MONTH = '$MONTH_NEXT' AND BANKCODE = '$BANK_CODE'";
                    ...
                    $this->db->where($where2);
                     $this->db->update('BANKBALANCE',$ins);
                     $this->db->trans_commit();
                    $this->db->flush_cache();
    ...
    function saveClosingNew($param,$address){
        ini_set("display_errors", 'On');
        try {
            foreach($param['DtClosing'] AS $key => $row) {
                // echo "<pre>";
                ...
                if($row["FLAG"] != 0){
                        ...
                        if ($result1) {
                            ...
                            // if($res){
                                ...
                                if($run){
                                    $result = true;
                                    $log = "INSERT INTO PAYMENT_PERIODCONTROLLOG SELECT * FROM PAYMENT_PERIODCONTROL WHERE SYSTEM = 'BANK' AND COMPANY = '$COMPANY'";
                                    $this->db->query($log);
                                    $this->db->trans_commit();
                                    $this->db->flush_cache();
                                }else{
