...
class PaymentModel extends BaseModel {
    ...
    public function DtOsPayment1($param) {
        ...
        $whereNodoc = '';
        if($NODOC != null || $NODOC != ''){
            $whereNodoc .= " AND (CFT.DOCNUMBER = '$NODOC' OR CFT.DOCNUMBER = '$NODOC' ) ";
        }
                    ...
                    FROM (SELECT CFT.*,
                            ...
                            FROM (SELECT CFT.ID,
                                  ...
                                  SELECT CFT.ID,
                                         ...
                                         CFT.INVOICEVENDORNO
                                    FROM CF_TRANSACTION CFT
                                   WHERE     CFT.DOCTYPE IN ('PDO', 'INV_AP_SPC','INV_AR_SPC','LEASING','LOAN')
                          ...
                          SELECT CFT.*,
                                 ...
                                 (  CFT.AMOUNTINVOICE
                                  ...
                                  - NVL (PAY.AMOUNTPAID, 0))
                                    AS AMOUNTFORECAST,
                                 (  CFT.AMOUNTINVOICE
                                  -- - NVL (FF.AMOUNTFORECAST, 0) karna payment, forecast bukan menjadi acuan utama untuk os payment
                                  - NVL (PAY.AMOUNTPAID, 0))
                                    AS AMOUNTOS,
                                 0 AS YEARX,
                                 0 AS MONTHX,
    ...
    public function DtOsPayment1Reversal($param) {
        $DATABY = "%" . $param["DATABY"] . "%";
        if ($param['DTPAY'] == '' || $param['DTPAY'] == NULL) {
            $param['DTPAY'] = "''";
        }
        ...
        $SQL = "SELECT DISTINCT CFT.*,
                 ...
                 (FF.AMOUNTADJS - NVL (PAY.AMOUNTPAID, 0)) AS AMOUNTFORECAST,
                 CASE
                    WHEN FF.AMOUNTADJS = 0 THEN FF.AMOUNTREQUEST
                    WHEN cft.AMOUNTINVOICE = NVL (PAY.AMOUNTPAID, 0) THEN 0
                    ELSE (FF.AMOUNTADJS - NVL (PAY.AMOUNTPAID, 0))
                 END
                    AS AMOUNTOS,
            ...
            FROM (SELECT CFT.ID,
                   ...
                   WHERE     (CFT.DOCTYPE LIKE '%REVERSAL')
                         AND CFT.COMPANY = '$COMPANY'
                         AND    CFT.company
                             || cft.docref
                             || cft.id
                             || TO_CHAR (cft.DOCDATE, 'yyyy') NOT IN
                                (SELECT DISTINCT
                                  ...
                                  WHERE     CFT.DOCNUMBER LIKE 'TMPINV%'
                                        AND CFT.COMPANY = '$COMPANY')
                         AND CFPO.NEGO_NO IS NOT NULL
                         AND CFT.DOCNUMBER NOT LIKE 'TMPINV%') CFT
                 INNER JOIN FORECAST_FIX FF
                    ON FF.CFTRANSID = CFT.ID AND FF.ISACTIVE = 2
                 INNER JOIN (SELECT ID, COMPANY, CURRENCY
                               FROM CF_TRANSACTION
                 ...
                 INNER JOIN USER_DEPART UD
                    ON UD.FCCODE = '$USERNAME' AND UD.DEPARTMENT = CFT.DEPARTMENT
                                 WHERE (FF.AMOUNTADJS - NVL (PAY.AMOUNTPAID, 0)) < 0 AND DT.CASHFLOWTYPE = '$CASHFLOWTYPE'
                         ) CFT
                         INNER JOIN COMPANY C ON C.ID = CFT.COMPANY
                         INNER JOIN BUSINESSUNIT BS
                            ON BS.ID = CFT.BUSINESSUNIT AND BS.COMPANY = CFT.COMPANY
                          ...
                          SELECT CFT.ID, NULL AS FORECASTID
                            FROM CF_TRANSACTION CFT
                                 ...
                                 (  SELECT FF.CFTRANSID,
                                  ...
                                  GROUP BY FF.CFTRANSID) FF
                                    ON FF.CFTRANSID = CFT.ID
                           WHERE     CFT.ID IN (" . $param['DTPAY'] . ")) PD
                            ON CONCAT (PD.ID, PD.FORECASTID) = CONCAT (CFT.ID, CFT.FORECASTID)
                   WHERE     CFT.DATABY LIKE '$DATABY'
                         AND CFT.CURRENCY = '$CURRENCY'
                         AND CFT.DOCNUMBER NOT LIKE 'TMPINV%'
    ...
    public function SavePaymentReversal($Data, $Location) {
        $date  = strtotime($Data['DATERELEASE']);
        $cTime = date("mY",$date);
        // var_dump($cTime);exit;
        ini_set('display_errors', 'On');
        $getVoucher = $this->db->get_where('PAYMENT',[ 'BANKCODE' => $Data['BANKCODE'],'VOUCHERNO' => $Data['VOUCHERNO']])->num_rows();
        if($getVoucher > 0){
            // if($getUserCompany == 0){
                throw new Exception("Duplicate Voucher");
            // }
        }
    ...
    //            Cek Amount Bank
                ...
                foreach ($Data["DtPaid"] as $values) {
                    ...
                    $dt["AMOUNT"] = intval(preg_replace("/[^\d\.\-]/","",$Data["AMOUNTPAID"]));
                    $dt["RATE"] = $Data["RATE"];
                    $dt["AMOUNTBANK"] = $Data["RATE"] * intval(preg_replace("/[^\d\.\-]/","",$Data["AMOUNTPAID"]));//$values["AMOUNTCONVERSI"];
                    $dt["REMARK"] = $values["REMARK"];
                    $dt["REF_VOUCHER"] = $Data['REF_VOUCHER'];
                    ...
                    //     $del = "DELETE FROM TASK_SCHEDULER WHERE DOCNUMBER = '".$values['DOCNUMBER']."' AND TO_CHAR(EMAILDATE,'mmyyyy') = '$cTime'";
                    //     $this->db->query($del);
                    // }
