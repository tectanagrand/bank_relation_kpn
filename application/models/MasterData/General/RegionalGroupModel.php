...
class RegionalGroupModel extends BaseModel {
    ...
    public function Save($Data, $Location) {
        try {
            $this->db->trans_begin();
            $result = FALSE;
            $SQL = "SELECT * FROM $this->REGIONGROUP WHERE FCCODE = ?";
            ...
            if ($Data['ACTION'] == 'ADD') {
                ...
                $result = $result->set($dt)->insert($this->REGIONGROUP);
            } elseif ($Data['ACTION'] == 'EDIT') {
                $result = $result->set($dt)
                        ->where(['FCCODE' => strtoupper($Data['FCCODE'])])
                        ->update($this->REGIONGROUP);
            }
            if ($result) {
    ...
    public function Delete($Data, $Location) {
        try {
            $this->db->trans_begin();
            $result = FALSE;
//            $result1 = $this->db->delete($this->REGIONGROUPCE, ['EXTSYSTEMCODE' => $Data['FCCODE']]);
            $result = $this->db->delete($this->REGIONGROUP, ['FCCODE' => $Data['FCCODE']]);
//            if ($result1 && $result2) {
//                $result = TRUE;
//            }
            if ($result) {
