...
class MstDeptPurchOrgModel extends BaseModel {
        ...
        try {
            foreach($param['dtPurch'] AS $key => $row) {
                ...
                if($row['DEPARTMENT'] == null || ''){
                    throw new Exception("Department cant null, check your data");
                }

                if($row['BUSINESSUNIT_CSF'] == null || ''){
                    throw new Exception("BUSINESS UNIT cant null, check your data");
                }

                $dt = [
                    'BUSINESSUNIT' => strtoupper($row['BUSINESSUNIT_CSF']),
                ...
                if($cekBU->num_rows() < 1){
                    ...
                    throw new Exception("Business Unit ".$row['BUSINESSUNIT_CSF']." Not Registered, Please Contact Your Administrator");
                }

                $cekDupe = "SELECT * FROM DEPARTMENT_PURCHORG WHERE BUSINESSUNIT = '".$row['BUSINESSUNIT_CSF']."' AND PURCHORG = '".$row['PURCHORG']."' AND DEPARTMENT = '".$row['DEPARTMENT']."'";
                // var_dump($cekBU);exit;
                $cekDupe = $this->db->query($cekDupe);

                if($cekDupe->num_rows() > 1){
                    throw new Exception("Business Unit ".$row['BUSINESSUNIT_CSF']." Already Registered");
                }
