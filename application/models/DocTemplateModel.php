...
class DocTemplateModel extends CI_Model {
    ...
    public function Save($Data, $Location) {
        try {
            $this->db->trans_begin();
            $result = FALSE;
            $SQL = "SELECT * FROM $this->table WHERE FCCODE <> ? AND FCNAME = ?";
            $Cek = $this->db->query($SQL, [$Data['FCCODE'], $Data['FCNAME']]);
            if ($Cek->num_rows() > 0) {
