<!-- begin breadcrumb -->
	<ol class="breadcrumb pull-right">
		...
		<li class="breadcrumb-item active">Dashboard Komidel</li>
	</ol>
<!-- end breadcrumb -->
<h1 class="page-header">Komidel<small></small></h1>
	...
	<!-- ================== END PAGE LEVEL JS ================== -->	
	<script src="<?php echo base_url()?>/assets/js/dashboard/bjr.min.js"></script>
	<!-- ================== END PAGE LEVEL JS ================== -->	
	...
	<script>
		$("#company").change(function(){
			if ($(this).val() != ''){
				...
				/*dropdown post *///  
				$.ajax({
					url : "<?php echo site_url('dashboardbjr/ajax_businessunit/')?>/" +  $(this).val(),
					type: "POST",
					dataType: "JSON",
		...
		$("#businessunit").change(function(){
			if ($(this).val() != ''){
				...
				$.ajax({
					url : "<?php echo site_url('dashboardbjr/ajax_division/')?>/" +  $(this).val(),
					type: "POST",
					dataType: "JSON",
		...
		$("#division").change(function(){
			if ($(this).val() != ''){
				...
				$.ajax({
					url : "<?php echo site_url('dashboardbjr/ajax_field/')?>/" +  $('.businessunit').val() + "/"+ $(this).val(),
					type: "POST",
					dataType: "JSON",
		...
		$(document).ready(function() {
			...
			Dashboard.init();
			$.ajax({
				  url : "<?php echo site_url().'/dashboardbjr/ajax_company';?>",
				  type: "POST",
				  dataType: "JSON",
