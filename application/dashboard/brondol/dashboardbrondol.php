<!-- begin breadcrumb -->
	<ol class="breadcrumb pull-right">
		...
		<li class="breadcrumb-item active">Dashboard brondolan</li>
	</ol>
<!-- end breadcrumb -->
<h1 class="page-header">Brondolan<small></small></h1>
	...
	<!-- ================== END PAGE LEVEL JS ================== -->	
	<script src="<?php echo base_url()?>/assets/js/dashboard/brondol.min.js"></script>
	<!-- ================== END PAGE LEVEL JS ================== -->	
	...
	<script>
		$("#company").change(function(){
			if ($(this).val() != ''){
				...
				/*dropdown post *///  
				$.ajax({
					url : "<?php echo site_url('dashboardbrondol/ajax_businessunit/')?>/" +  $(this).val(),
					type: "POST",
					dataType: "JSON",
		...
		$("#businessunit").change(function(){
			if ($(this).val() != ''){
				...
				$.ajax({
					url : "<?php echo site_url('dashboardbrondol/ajax_division/')?>/" +  $(this).val(),
					type: "POST",
					dataType: "JSON",
		...
		$("#division").change(function(){
			if ($(this).val() != ''){
				...
				$.ajax({
					url : "<?php echo site_url('dashboardbrondol/ajax_field/')?>/" +  $('.businessunit').val() + "/"+ $(this).val(),
					type: "POST",
					dataType: "JSON",
		...
		$(document).ready(function() {
			...
			Dashboard.init();
			$.ajax({
				  url : "<?php echo site_url().'/dashboardbrondol/ajax_company';?>",
				  type: "POST",
				  dataType: "JSON",
