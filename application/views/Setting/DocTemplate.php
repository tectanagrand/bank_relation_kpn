...
<h1 class="page-header">Document Template</h1>
<div class="panel panel-success">
    ...
    <div class="panel-heading">
        ...
        <h4 class="panel-title">Document Template</h4>
    </div>
    <div class="panel-body">
        <?php if (empty($_GET)) { ?>
            ...
            <div class="row m-0 table-responsive">
                <table id="DtTemplate" class="table table-bordered table-striped table-hover dataTable no-footer dtr-inline" role="grid" width="100%" aria-describedby="DtTemplate_info" style="width: 100%;">
                    <thead>
                        <tr role="row">
                            <th class="text-center sorting_asc" style="width: 30px;">No</th>
                            <th class="text-center sorting">Doc. Template Code</th>
                            <th class="text-center sorting">Doc. Template Name</th>
            ...
            <form id="FAddEditForm" data-parsley-validate="true" data-parsley-errors-messages-disabled onsubmit="return false">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="fccode">Doc. Template Code *</label>
                        <input type="text" class="form-control" name="FCCODE" id="FCCODE" placeholder="Doc. Template Code" required>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="fcname">Doc. Template Name *</label>
                        <input type="text" class="form-control" name="FCNAME" id="FCNAME" placeholder="Doc. Template Name" required>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-6">
...
<script>
    ...
    $(document).ready(function () {
        if (getUrlParameter('type') == "edit" || getUrlParameter('type') == "add") {
            ...
            if (getUrlParameter('type') == "add") {
                ...
                var data = <?php echo json_encode($DtTemplate); ?>;
                SetData(data);
            }
        } else {
            if (!$.fn.DataTable.isDataTable('#DtTemplate')) {
                $('#DtTemplate').DataTable({
                    "processing": true,
                    "ajax": {
                        "url": "<?php echo site_url('IDocTemplate/ShowData') ?>",
                ...
                $('#DtTemplate thead th').addClass('text-center');
                table = $('#DtTemplate').DataTable();
                ...
                table.on('click', '.delete', function () {
                    ...
                    if (confirm('Are you sure delete this data "'+ data.FCNAME +'" ?')) {
                        $.ajax({
                            dataType: "JSON",
                            type: "POST",
                            url: "<?php echo site_url('IDocTemplate/Delete'); ?>",
                            data: {
                                FCCODE: data.FCCODE,
                                USERNAME: USERNAME
                            },
    ...
    function SetDataKosong() {
        $('.panel-title').text('Add Data Document Template');
        $('#FCCODE').val('');
        $('#FCNAME').val('');
        $('#DESCRIPTION').val('');
        $('#ISACTIVE').val('TRUE');
    ...
    function SetData(data) {
        $('.panel-title').text('Edit Data Document Template');
    ...
    var Save = function () {
        if ($('#FAddEditForm').parsley().validate()) {
            ...
            $.ajax({
                dataType: "JSON",
                type: "POST",
                url: "<?php echo site_url('IDocTemplate/Save'); ?>",
                data: {
                    FCCODE: $('#FCCODE').val(),
                    FCNAME: $('#FCNAME').val(),
