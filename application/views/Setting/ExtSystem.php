...
<h1 class="page-header">External System</h1>
    ...
    <div class="panel-body">
        <?php if (empty($_GET)) { ?>
            ...
            <div class="row m-0 table-responsive">
                <table id="DtSystem" class="table table-bordered table-striped table-hover dataTable no-footer dtr-inline" role="grid" width="100%" aria-describedby="DtSystem_info" style="width: 100%;">
                    <thead>
                        <tr role="row">
            ...
            <form id="FAddEditForm" data-parsley-validate="true" data-parsley-errors-messages-disabled onsubmit="return false">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="fccode">Ext. System Code *</label>
                        <input type="text" class="form-control" name="FCCODE" id="FCCODE" placeholder="Ext. System Code" required>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="fcname">Ext. System Name *</label>
                        <input type="text" class="form-control" name="FCNAME" id="FCNAME" placeholder="Ext. System Name" required>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-6">
...
<script>
    ...
    $(document).ready(function () {
        if (getUrlParameter('type') == "edit" || getUrlParameter('type') == "add") {
            ...
            if (getUrlParameter('type') == "add") {
                ...
                var data = <?php echo json_encode($DtSystem); ?>;
                SetData(data);
            }
        } else {
            if (!$.fn.DataTable.isDataTable('#DtSystem')) {
                $('#DtSystem').DataTable({
                    "processing": true,
                    "ajax": {
                        "url": "<?php echo site_url('IExtSystem/ShowData') ?>",
    ...
    function SetDataKosong() {
        $('.panel-title').text('Add Data External System');
        $('#FCCODE').val('');
        $('#FCNAME').val('');
        $('#DESCRIPTION').val('');
        $('#ISACTIVE').val('TRUE');
    ...
    function SetData(data) {
        $('.panel-title').text('Edit Data External System');
    ...
    var Save = function () {
        if ($('#FAddEditForm').parsley().validate()) {
            ...
            $.ajax({
                dataType: "JSON",
                type: "POST",
                url: "<?php echo site_url('IExtSystem/Save'); ?>",
                data: {
                    FCCODE: $('#FCCODE').val(),
                    FCNAME: $('#FCNAME').val(),
