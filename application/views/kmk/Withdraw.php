                ...
                <table id="DtLeasing" class="table table-bordered table-striped table-hover dataTable no-footer dtr-inline" role="grid" width="100%" aria-describedby="DtLeasing_info">
                    <thead>
                        <tr role="row">
                            ...
                            <th class="text-center sorting">Maksimal Limit</th>
                            <th class="text-center sorting">Sisa Limit</th>
                            <th class="text-center sorting_disabled" aria-label="Action">#</th>
                        </tr>
                    </thead>
                </table>
            ...
            if (!$.fn.DataTable.isDataTable('#DtLeasing')) {
                $('#DtLeasing').DataTable({
                    ...
                    "columns": [
                    {
                        ...
                        render : function (data, type, row, meta) {
                            // console.log(data);
                            if(row.CREDIT_TYPE == 'KI'){
                                var html = '<button class="btn btn-primary btn-icon btn-circle btn-sm editdet showChild" title="" style="margin-right: 5px;">\n\
                    ...
                    // {"data": "SUB_CREDIT_TYPE_1"},
                    {
                        ...
                        render: function (data,type,row,meta) {
                            ...
                            else if(row.CREDIT_TYPE === 'KI') {
                                return '-- See Detail (+) --'
                            } 
                            else {
                                return row.CURRENCY + ' ' + renderer(row.AMOUNT_LIMIT);
                            }
                        }
                    },
                    {
                        ...
                        render: function (data,type,row,meta) {
                            if (data == null) {
                                ...
                                return row.CURRENCY + ' ' + renderer(row.AMOUNT_BALANCE);
                            }
                        }
                    },
                    {
                ...
                table.on('click', '.edit', function() {
                    ...
                    if(data.SUB_CREDIT_TYPE == 'WA' && data.SUB_CREDIT_TYPE_1 == 'WA'){
                        window.open("<?php echo site_url('WithdrawWA'); ?>" + '?type=edit&UUID=' + data.UUID,'_blank');   
                    }
                    if(data.SUB_CREDIT_TYPE == 'WA' && data.SUB_CREDIT_TYPE_1 == 'KMK_SCF_AR'){
                        window.open("<?php echo site_url('WithdrawAR'); ?>" + '?type=edit&UUID=' + data.UUID,'_blank');   
                    }
                    if(data.SUB_CREDIT_TYPE == 'WA' && data.SUB_CREDIT_TYPE_1 == 'KMK_SCF_AP'){
                        window.open("<?php echo site_url('WithdrawAP'); ?>" + '?type=edit&UUID=' + data.UUID,'_blank');   
                    }
                    if(data.SUB_CREDIT_TYPE == 'KI'){
                        window.open("<?php echo site_url('WithdrawKI'); ?>" + '?type=edit&UUID=' + data.UUID,'_blank');   
                    }
...
<script type="text/javascript">
    ...
    function createChild(row, data) {
        ...
        var childTable = table.DataTable({
            "processing": true,
            "ajax": {
                "url": "<?php echo site_url('Kmk/getOWTrancheBalanceKI')?>",
                dataType: "JSON",
                type: "POST",
                "data": {
                    UUID : data.UUID,
                ...
                "columns" : [
                    ...
                    },
                    {
                        ...
                        "data"  : "LIMIT_TRANCHE",
                        render: function (data, type, row, meta) {
                            return row.CURRENCY + ' ' + renderer(row.LIMIT_TRANCHE)
                        }   
                    },
                    {
                        "title" : "Sisa Limit",
    ...
    function updateBalanceAfterPaid () {
        $.ajax({
            dataType: 'JSON',
            type:'POST',
            url: "<?php echo site_url('Kmk/updateBalanceAfterPaid')?>",
            data: '',
            success: function (response) {
                var data = response.result.data;
                var table = $('#DtLeasing').DataTable();
                var notpaid = false ;
                try {
                    if(data.NOTPAID == 'TRUE') {
                        notpaid = true ;
                    }
                } catch {
                    notpaid = false;
    ...
    setInterval(updateBalanceAfterPaid, 1000*60);
    //
    // DisableBtn();
</script>