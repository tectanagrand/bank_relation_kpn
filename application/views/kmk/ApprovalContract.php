...
<div class="panel panel-success">
    ...
    <div class="panel-heading">
        ...
        <h4 class="panel-title">Approval Contract</h4>
    </div>
    <div class="panel-body">
        <?php if (empty($_GET)) { ?>
            <div class="row mb-2">
                <div class="col-md-8 pull-left">
                    <h3>Approval Contract</h3>
                </div>
        ...
        if (getUrlParameter('type') == "edit" || getUrlParameter('type') == "add") {
            ...
            if (!$.fn.DataTable.isDataTable('#DtLeasing')) {
                $('#DtLeasing').DataTable({
                    "processing": true,
                    "ajax": {
                        "url": "<?php echo site_url('Kmk/showApprovalContract') ?>",
                        dataType: "JSON",
                        type: "POST",
                ...
                table.on('click', '.accept', function () {
                    ...
                    if (confirm('Are you sure confirm this data ?')) {
                        $.ajax({
                            dataType: "JSON",
                            type: "POST",
                            url: "<?php echo site_url('Kmk/acceptContract'); ?>",
                            data: {
                                UUID: data.UUID,
                ...
                table.on('click', '.edit', function() {
                    ...
                    if(data.SUB_CREDIT_TYPE == 'RK'){
                        window.open("<?php echo site_url('KMKMasterRKBD'); ?>" + '?type=edit&UUID=' + data.UUID + '&vm=vm','_self');    
                    }
                    ...
                    if(data.SUB_CREDIT_TYPE == 'WA'){
                        window.open("<?php echo site_url('KMKMasterWA'); ?>" + '?type=edit&UUID=' + data.UUID + '&vm=vm','_self');   
                    }
                    if(data.SUB_CREDIT_TYPE == 'KI' ){
                        window.open("<?php echo site_url('KMKMasterKI'); ?>" + '?type=edit&UUID=' + data.UUID + '&vm=vm','_self');   
                    }
