    ...
    <div class="panel-body">
        <ul class="nav nav-tabs">
            <li class="nav-item">
                <a href="#scfndk" data-toggle="tab" class="leasingdata nav-link active">
                    ...
                    <span class="d-sm-block d-none">KMK SCF AP </span>
                </a>
            </li>
        </ul>
        ...
        <div class="panel panel-success bd d-none">
            <div class="panel panel-default">
                <div class="panel-body">
                    <form id="FSCFAP" data-parsley-validate="true" data-parsley-errors-messages-disabled="" onsubmit="return false" novalidate="">
                    <input id="DATA_EXIST" hidden/>
                        ...
                        <div class="row">
                            <div class="form-group col-md-4">
                                    ...
                                    <input type="text" class="form-control" name="EXC_RATE" id="EXC_RATE" placeholder="exc. rate" required/>
                            </div>
                            <div class="form-group col-md-4">
                                    <label for="fcname">Interest % *</label>
                                    <input type="number" class="form-control" name="INTEREST" id="INTEREST" placeholder="interest" step="0.00001" required/>
                            </div>
                            <div class="form-group col-md-4">
                                    <label for="fcname">Provision % *</label>
                                    <input type="number" class="form-control" name="PROVISION" id="PROVISION" placeholder="provision" step="0.00001" required/>
                            </div>
                        </div>
                        <div class="row">
        ...
        <!-- form detail -->
            ...
            <div class="row m-0 table-responsive">
                <table id="Table_DtWDAP" class="table table-bordered table-hover dataTable" role="grid" width="100%" style="width: 100%;">
                    <thead>
                        <tr role="row">
                            <th class="text-center align-middle">No</th>
                            <th class="text-center align-middle">Vendor</th>
                            <th class="text-center align-middle">Vendor Bank</th>
                            <th class="text-center align-middle">Vendor Bank Account</th>
                            <th class="text-center align-middle">Invoice Date</th>
                            <th class="text-center align-middle">Invoice Number</th>
                            <th class="text-center align-middle">Invoice Value</th>
        ...
        <div class="row mt-2 fileupload-buttonbar">
                        <div class="col-md-4">
                            ...
                            <span class="btn btn-primary fileinput-button m-r-3">
                                ...
                                <input id='upload-button' type="file" class="upload-file" data-max-size="1048576" onchange="filesChange(this)">
                            </span>
     ...
     <div class="modal fade" id="MDetail">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form id="SCFAPDetail" data-parsley-validate="true" data-parsley-errors-messages-disabled  enctype="multipart/form-data" onsubmit="return false">
                    <div class="modal-body">
                        <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>Vendor <span id="BIC"></span></label>
                                        <br/>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="VENDOR_BANK" name="VENDOR_BANK" />
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>Vendor Bank Account</label>
                                        <input type="text" class="form-control" id="V_BANK_ACC" name="V_BANK_ACC" />
                                    </div>
                                </div>
                        </div>
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="INV_NUM" name="INV_NUM"/>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>Invoice Value</label>
                                    <input type="text" class="form-control" id="INV_VALUE" name="INV_VALUE" data-type="currency" required/>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary" id="SAVE_WDAR" name="SAVE_WDAR" onclick=saveWDAP(ATTACHMENT_WDAR)>Save</button>
...
<script>
    ...
    $('#VALUE_DATE').on("change keyup", function() {
        ...
        $.ajax({
            dataType: "JSON",
            type: "POST",
            url: "<?php echo site_url('Kmk/GetDataWDAP');?>",
            data: {
                UUID: DATASCFAR.UUID,
                WD_TYPE: "KMK_SCF_AP"
            },
    ...
    function SetData(data) {
        ...
        $('#EXC_RATE').val(fCurrency(data.RATE));
        $('#PROVISION').val(data.PROVISION);
        $('#INTEREST').val(data.INTEREST);
        $('#LIMIT_WA').val(fCurrency(data.LIMIT_WA));
        var base = Number(data.LIMIT_WA) ;
        var converted = Number(data.RATE) * base ;
        ...
        $.ajax({
            ...
            success : function(r) {
                data = r.result.data;
        // Block code to update Amount Balance from Views
        if (data.SUB_BALANCE == null || data.SUB_BALANCE == 0 || data.SUB_BALANCE == '') {
                    var currLimit = DtKmk.AMOUNT_LIMIT - (DtKmk.AMOUNT ? DtKmk.AMOUNT : 0) ;
                    BALANCE = currLimit ;
            $('#BALANCE').val(fCurrency(String(currLimit)));    
                    // console.log("no data");       
        } else {
                    var currLimit = data.SUB_BALANCE - ((DtKmk.AMOUNT && DtKmk.STATUS == 0)  ? DtKmk.AMOUNT : 0) ;
                    BALANCE = currLimit ;
                    $('#BALANCE').val(fCurrency(String(currLimit)));
                    // console.log("data");
        }
    ...
    var SaveWD = function () {
        if ($('#FSCFAP').parsley().validate()) {
                ...
                $.ajax({
                    ...
                    data: {
                        UUID: ID,
                        BATCHID : DATASCFAR.BATCHID,
                        WD_TYPE: DATASCFAR.SUB_WD_TYPE,
                        USERNAME: USERNAME,
                        RATE: $('#EXC_RATE').val(),
...
<script type="text/javascript">
    ...
    if (!$.fn.DataTable.isDataTable('#DtUpload')) {
        $('#DtUpload').DataTable({
            ...
            "ajax": {
                    ...
                    "data": function(d) {
                        d.UUID = DtUpload;
                        d.SUB_CREDIT_TYPE = DtKmk['WD_TYPE'];   
    ...
    function saveWDAP(elm) {
        if($('#SCFAPDetail').parsley().validate()){
                ...
                if ($.inArray(extFile, filetypeUpload) === -1) {
                    ...
                    if (fileInput.get(0).files.length) {
                        ...
                        if (fileSize > maxSize) {
                            ...
                            fd.append("BATCHID", DATASCFAR.BATCHID);
                            fd.append("USERNAME", USERNAME);
                            fd.append('ID', $('#DETID').val());
                            fd.append("UUID",ID);
                            fd.append("VENDOR", $('#VENDOR').val());
                            fd.append("FILENAME", FILENAME);
                            fd.append("VENDOR_BANK",$('#VENDOR_BANK').val());
                            fd.append("V_BANK_ACC",  $("#V_BANK_ACC").val());
                            fd.append("INV_DATE", $('#INV_DATE').val());
                            fd.append("INV_NUM",  $("#INV_NUM").val());
                            fd.append("INV_VALUE",  $("#INV_VALUE").val());
                            fd.append("DDOWN_AMT", Number((formatDesimal($("#DDOWN_AMT").val()))));
                            fd.append("DUE_DATE", $("#DUE_DATE").val());
                            fd.append("DISKONTO", $("#DISKONTO").val());
                            fd.append("AMTMODALS", amtModals ) ;
                            fd.append("WD_TYPE", "KMK_SCF_AP");
                            // fd.append("EXTSYSTEM",$('#EXTSYSTEM').val());
                            // fd.append("DOCTYPE",$('#DOCTYPE').val());
                            // fd.append('UUID',UUID)
                            // fd.append('DATERELEASE',currentDate);
                            $.ajax({
                                dataType: "JSON",
                                type: 'POST',
                                url: "<?php echo site_url('Kmk/SaveWDAPDet'); ?>",
                                data: fd,
                                processData: false,
                                contentType: false,
                                success: function (response) {
...
1
    $('#INV_VALUE').on('change keyup', function () {
        var bilVal = Number(formatDesimal($('#INV_VALUE').val()));
    ...
    $('#VENDOR').on('change', function() {
        ...
        $.ajax({
            dataType: 'JSON',
            type : 'POST',
            url:"<?php echo site_url('Kmk/getVendorAP') ?>",
            data:{
                ID : vendorID
            },
            success: function(o) {
                data = o.result.data;
                // console.log(data);
                $('#VENDOR_BANK').val(data.BANKNAME);
                $('#V_BANK_ACC').val(data.BANKACCOUNT);
            },
            error: function(e) {
                alert('Error !');
            }
    ...
    // $('#DISKONTO').on('change', function () {
    //     var bilVal = formatDesimal($('#INV_VALUE').val());
    //     var diskonto = formatDesimal($('#DISKONTO').val());
    //     var ddown_amt = Number(bilVal) * 95/100
    //     var DDOWN_AMT = Number(ddown_amt)
    //     $('#DDOWN_AMT').val(fCurrency(String(DDOWN_AMT)));
    // });
    ...
    if (!$.fn.DataTable.isDataTable('#Table_DtWDAP')) {
        $('#Table_DtWDAP').DataTable({
            "bDestroy" : true,
            "bRetrieve" : true,
            "ajax": {
                    "url" : "<?php echo site_url('Kmk/GetDataWDAPDet')?>",
                    "type": "POST",
                    "datatype" : "JSON",
                    "data": function(d) {
                        d.UUID = DATASCFAR.BATCHID;
                        d.VALUE_DATE = DtKmk.VALUE_DATE ? DtKmk.VALUE_DATE : $('#VALUE_DATE').val();
                    },
                    "dataSrc": function(ext) {
                        if (ext.status == 200) {
                            // BEGIN = 0;
            ...
            "columns": [
                ...
                {"className" : "text-center", "data":"V_BANK_ACC"},
                {"className" : "text-center", "data":"INV_DATE"},
                {"className" : "text-center", "data":"INV_NUM"},
                {"className" : "text-center", "data":"INV_VALUE",
                    render: $.fn.dataTable.render.number(',', '.', 2)},
        ...
        tbl_WDAP = $('#Table_DtWDAP').DataTable();

        //assigning amount for edit or add
        $('#btnDetail').on("click", function() {
            $('#DETID').val(null);
            amtModals = 0;    
        }); 
        //
        tbl_WDAP.on('click', '.edit', function() {
            $tr = $(this).closest('tr');
            var data = tbl_WDAP.row($tr).data();
            DETID = data.ID;
            ...
            $('#VENDOR_BANK').val(data.VENDOR_BANK);
            $('#V_BANK_ACC').val(data.V_BANK_ACC);
            $('#INV_DATE').val(moment(data.INV_DATE).format('YYYY-MM-DD'));
            $('#INV_NUM').val(data.INV_NUM);
            $('#INV_VALUE').val(fCurrency(data.INV_VALUE));
        ...
        tbl_WDAP.on('click', '.dwn', function() {
            $tr = $(this).closest('tr');
            var data = tbl_WDAP.row($tr).data();
            window.open("<?php echo base_url('assets/file/')?>" + data.FILE_NAME,'_blank');
        });
        tbl_WDAP.on('click', '.delete', function () {
                $tr = $(this).closest('tr');
                var data = tbl_WDAP.row($tr).data();
                ...
                if (confirm('Are you sure delete this data ?')) {
                    $.ajax({
                        dataType: "JSON",
                        type: "POST",
                        url: "<?php echo site_url('Kmk/DeleteAP'); ?>",
                        data: {
                            ID: data.ID,
        ...
        // $('#F_VALUE_DATE').on('keyup change', function() {
        //     console.log($('#F_VALUE_DATE').val());
        //     tbl_WDAP.ajax.reload();
        // });
    }
