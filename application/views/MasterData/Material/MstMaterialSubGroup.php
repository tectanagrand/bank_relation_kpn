<ol class="breadcrumb pull-right">
    ...
    <li class="breadcrumb-item active">Master Material Sub Group</li>
</ol>
<h1 class="page-header">Master Material Sub Group</h1>
<div class="panel panel-success">
    ...
    <div class="panel-heading">
        ...
        <h4 class="panel-title">Master Material Sub Group</h4>
    </div>
                ...
                <table id="DtMaterial" class="table table-striped table-bordered" cellspacing="0" width="100%" aria-describedby="DtMaterial_info">
                    <thead>
                        <tr role="row">
                            ...
                            <th class="text-center sorting">Material Sub Code</th>
                            <th class="text-center sorting">Material Sub Name</th>
            ...
            <form id="FAddEditForm" data-parsley-validate="true" data-parsley-errors-messages-disabled onsubmit="return false">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="fccode">Material Sub Code *</label>
                        <input type="text" class="form-control" name="FCCODE" id="FCCODE" placeholder="Material Sub Code" required>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="fcname">Material Sub Name *</label>
                        <input type="text" class="form-control" name="FCNAME" id="FCNAME" placeholder="Material Sub Name" required>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-6">
...
<script>
    ...
    $(document).ready(function () {
        if (getUrlParameter('type') == "edit" || getUrlParameter('type') == "add") {
            ...
            if (getUrlParameter('type') == "add") {
                ...
                var data = <?php echo json_encode($DtMaterialSub); ?>;
                SetData(data);
            }
        } else {
            if (!$.fn.DataTable.isDataTable('#DtMaterial')) {
                $('#DtMaterial').DataTable({
                    "processing": true,
                    "ajax": {
                        "url": "<?php echo site_url('IMaterialSubGroup/ShowData') ?>",
                ...
                table.on('click', '.delete', function () {
                    ...
                    if (confirm('Are you sure delete this data "' + data.FCNAME + '" ?')) {
                        $.ajax({
                            dataType: "JSON",
                            type: "POST",
                            url: "<?php echo site_url('IMaterialSubGroup/Delete'); ?>",
                            data: {
                                FCCODE: data.FCCODE,
                                USERNAME: USERNAME
                            },
    ...
    function SetDataKosong() {
        $('.panel-title').text('Add Data Material Sub Group');
        $('#FCCODE').val('');
        $('#FCCODE').val('');
        $('#FCNAME').val('');
        $('#DESCRIPTION').val('');
    ...
    function SetData(data) {
        $('.panel-title').text('Edit Data Material Sub Group');
    ...
    var Save = function () {
        if ($('#FAddEditForm').parsley().validate()) {
            ...
            $.ajax({
                dataType: "JSON",
                type: "POST",
                url: "<?php echo site_url('IMaterialSubGroup/Save'); ?>",
                data: {
                    FCCODE: $('#FCCODE').val(),
                    FCNAME: $('#FCNAME').val(),
