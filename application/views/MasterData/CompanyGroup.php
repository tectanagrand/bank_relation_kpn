...
<h1 class="page-header">Company Group</h1>
<div class="panel panel-success">
    ...
    <div class="panel-heading">
        ...
        <h4 class="panel-title">Company Group</h4>
    </div>
    <div class="panel-body">
        <?php if (empty($_GET)) { ?>
            ...
            <div class="row m-0 table-responsive">
                <table id="DtCompanyGroup" class="table table-bordered table-striped table-hover dataTable no-footer dtr-inline" role="grid" width="100%" aria-describedby="DtCompanyGroup_info" style="width: 100%;">
                    <thead>
                        <tr role="row">
                            <th class="text-center sorting_asc" style="width: 30px;">No</th>
                            <th class="text-center sorting">Comp. Group Code</th>
                            <th class="text-center sorting">Comp. Group Name</th>
            ...
            <form id="FAddEditForm" data-parsley-validate="true" data-parsley-errors-messages-disabled onsubmit="return false">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="fccode">Comp. Group Code *</label>
                        <input type="text" class="form-control" name="FCCODE" id="FCCODE" placeholder="Comp. Group Code" required>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="fcname">Comp. Group Name *</label>
                        <input type="text" class="form-control" name="FCNAME" id="FCNAME" placeholder="Comp. Group Name" required>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-6">
    ...
    $(document).ready(function () {
        if (getUrlParameter('type') == "edit" || getUrlParameter('type') == "add") {
            ...
            if (getUrlParameter('type') == "add") {
                ...
                var data = <?php echo json_encode($DtCompanyGroup); ?>;
                SetData(data);
            }
        } else {
            if (!$.fn.DataTable.isDataTable('#DtCompanyGroup')) {
                $('#DtCompanyGroup').DataTable({
                    "processing": true,
                    "ajax": {
                        "url": "<?php echo site_url('ICompanyGroup/ShowData') ?>",
                ...
                $('#DtCompanyGroup thead th').addClass('text-center');
                table = $('#DtCompanyGroup').DataTable();
                ...
                table.on('click', '.delete', function () {
                    ...
                    if (confirm('Are you sure delete this data "' + data.FCNAME + '" ?')) {
                        $.ajax({
                            dataType: "JSON",
                            type: "POST",
                            url: "<?php echo site_url('ICompanyGroup/Delete'); ?>",
                            data: {
                                FCCODE: data.FCCODE,
                                USERNAME: USERNAME
                            },
    ...
    var Save = function () {
        if ($('#FAddEditForm').parsley().validate()) {
            ...
            $.ajax({
                dataType: "JSON",
                type: "POST",
                url: "<?php echo site_url('ICompanyGroup/Save'); ?>",
                data: {
                    FCCODE: $('#FCCODE').val(),
                    FCNAME: $('#FCNAME').val(),
