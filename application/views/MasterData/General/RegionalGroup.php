<ol class="breadcrumb pull-right">
    ...
    <li class="breadcrumb-item active">Region Group</li>
</ol>
<h1 class="page-header">Region Group</h1>
<div class="panel panel-success">
    ...
    <div class="panel-heading">
        ...
        <h4 class="panel-title">Region Group</h4>
    </div>
    <div class="panel-body">
        <?php if (empty($_GET)) { ?>
            ...
            <div class="row m-0 table-responsive">
                <table id="DtRegionalGroup" class="table table-striped table-bordered table-hover dataTable no-footer dtr-inline" role="grid" width="100%" aria-describedby="DtRegionalGroup_info" style="width: 100%;">
                    <thead>
                        <tr role="row">
                            <th class="text-center sorting_asc" style="width: 30px;">No</th>
                            <th class="text-center sorting">Reg. Group Code</th>
                            <th class="text-center sorting">Reg. Group Name</th>
            ...
            <form id="FAddEditForm" data-parsley-validate="true" data-parsley-errors-messages-disabled onsubmit="return false">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="fccode">Reg. Group Code *</label>
                        <input type="text" class="form-control" name="FCCODE" id="FCCODE" placeholder="Reg. Group Code" required>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="fcname">Reg. Group Name *</label>
                        <input type="text" class="form-control" name="FCNAME" id="FCNAME" placeholder="Reg. Group Name" required>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-6">
    ...
    $(document).ready(function () {
        if (getUrlParameter('type') == "edit" || getUrlParameter('type') == "add") {
            ...
            if (getUrlParameter('type') == "add") {
                ...
                var data = <?php echo json_encode($DtRegionalGroup); ?>;
                SetData(data);
            }
        } else {
            if (!$.fn.DataTable.isDataTable('#DtRegionalGroup')) {
                $('#DtRegionalGroup').DataTable({
                    "processing": true,
                    "ajax": {
                        "url": "<?php echo site_url('IRegionalGroup/ShowData') ?>",
                ...
                $('#DtRegionalGroup thead th').addClass('text-center');
                table = $('#DtRegionalGroup').DataTable();
                ...
                table.on('click', '.delete', function () {
                    ...
                    if (confirm('Are you sure delete this data "' + data.FCNAME + '" ?')) {
                        $.ajax({
                            dataType: "JSON",
                            type: "POST",
                            url: "<?php echo site_url('IRegionalGroup/Delete'); ?>",
                            data: {
                                FCCODE: data.FCCODE,
                                USERNAME: USERNAME
                            },
    ...
    var Save = function () {
        if ($('#FAddEditForm').parsley().validate()) {
            ...
            $.ajax({
                dataType: "JSON",
                type: "POST",
                url: "<?php echo site_url('IRegionalGroup/Save'); ?>",
                data: {
                    FCCODE: $('#FCCODE').val(),
                    FCNAME: $('#FCNAME').val(),
