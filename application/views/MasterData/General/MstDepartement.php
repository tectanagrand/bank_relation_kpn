...
<h1 class="page-header">Master Departement</h1>
<div class="panel panel-success">
    ...
    <div class="panel-heading">
        ...
        <h4 class="panel-title">Master Departement</h4>
    </div>
                ...
                <table id="DtSystem" class="table table-striped table-bordered" cellspacing="0" width="100%" aria-describedby="DtSystem_info">
                    <thead>
                        <tr role="row">
                            ...
                            <th class="text-center sorting">Departement Code</th>
            ...
            <form id="FAddEditForm" data-parsley-validate="true" data-parsley-errors-messages-disabled onsubmit="return false">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="fccode">Departement Code *</label>
                        <input type="text" class="form-control" name="FCCODE" id="FCCODE" placeholder="Departement Code" required>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="fcname">Departement Name *</label>
                        <input type="text" class="form-control" name="FCNAME" id="FCNAME" placeholder="Departement Name" required>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-6">
...
<script>
        ...
        if (getUrlParameter('type') == "edit" || getUrlParameter('type') == "add") {
            ...
            if (!$.fn.DataTable.isDataTable('#DtSystem')) {
                $('#DtSystem').DataTable({
                    "processing": true,
                    "ajax": {
                        "url": "<?php echo site_url('IDepartement/ShowData') ?>",
                ...
                table.on('click', '.delete', function () {
                    ...
                    if (confirm('Are you sure delete this data "' + data.FCNAME + '" ?')) {
                        $.ajax({
                            dataType: "JSON",
                            type: "POST",
                            url: "<?php echo site_url('IDepartement/Delete'); ?>",
                            data: {
                                FCCODE: data.FCCODE,
                                USERNAME: USERNAME
                            },
    ...
    function SetDataKosong() {
        $('.panel-title').text('Add Data Departement');
        $('#FCCODE').val('');
        $('#FCNAME').val('');
        $('#DESCRIPTION').val('');
        $('#ISACTIVE').val('TRUE');
    ...
    function SetData(data) {
        $('.panel-title').text('Edit Data Departement');
    ...
    var Save = function () {
        if ($('#FAddEditForm').parsley().validate()) {
            ...
            $.ajax({
                dataType: "JSON",
                type: "POST",
                url: "<?php echo site_url('IDepartement/Save'); ?>",
                data: {
                    FCCODE: $('#FCCODE').val(),
                    FCNAME: $('#FCNAME').val(),
