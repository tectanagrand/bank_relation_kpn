    ...
    <div class="panel-body">
        <?php if (empty($_GET)) { ?>
            ...
            <div class="row m-0 table-responsive">
                <table id="DtRegional" class="table table-bordered table-striped table-hover dataTable no-footer dtr-inline" role="grid" width="100%" aria-describedby="DtRegional_info" style="width: 100%;">
                    <thead>
                        <tr role="row">
                            <th class="text-center sorting_asc" style="width: 30px;">No</th>
                            <th class="text-center sorting">Region Group</th>
                            <th class="text-center sorting">Region Code</th>
                            <th class="text-center sorting">Region Name</th>
            ...
            <form id="FAddEditForm" data-parsley-validate="true" data-parsley-errors-messages-disabled onsubmit="return false">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="fccode">Region Code *</label>
                        <input type="text" class="form-control" name="FCCODE" id="FCCODE" placeholder="Region Code" required>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="fcname">Region Name *</label>
                        <input type="text" class="form-control" name="FCNAME" id="FCNAME" placeholder="Region Name" required>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-6">
                    ...
                    <div class="form-group col-md-6">
                        <label for="description">Region Group</label>
                        <select class="form-control" id="REGIONGROUP" required>
                            <option value="" selected disabled>Choose Regional Group</option>
                            <?php
                            foreach ($DtRegionalGroup as $values) {
    ...
    $(document).ready(function () {
        if (getUrlParameter('type') == "edit" || getUrlParameter('type') == "add") {
            ...
            if (getUrlParameter('type') == "add") {
                ...
                var data = <?php echo json_encode($DtRegional); ?>;
                SetData(data);
            }
        } else {
            if (!$.fn.DataTable.isDataTable('#DtRegional')) {
                $('#DtRegional').DataTable({
                    "processing": true,
                    "ajax": {
                        "url": "<?php echo site_url('IRegional/ShowData') ?>",
                ...
                $('#DtRegional thead th').addClass('text-center');
                table = $('#DtRegional').DataTable();
    ...
    var Save = function () {
        if ($('#FAddEditForm').parsley().validate()) {
            ...
            $.ajax({
                dataType: "JSON",
                type: "POST",
                url: "<?php echo site_url('IRegional/Save'); ?>",
                data: {
                    FCCODE: $('#FCCODE').val(),
                    FCNAME: $('#FCNAME').val(),
