...
<h1 class="page-header">Material Group</h1>
<div class="panel panel-success">
    ...
    <div class="panel-heading">
        ...
        <h4 class="panel-title">Material Group</h4>
    </div>
    <div class="panel-body">
                ...
                <table id="DtMaterial" class="table table-striped table-bordered" cellspacing="0" width="100%" aria-describedby="DtMaterial_info">
                    <thead>
                        <tr role="row">
                            ...
                            <th class="text-center sorting">Sub 1</th>
                            <th class="text-center sorting">Sub 2</th>
        ...
        <?php } else { ?>
            <form id="FAddEditForm" data-parsley-validate="true" data-parsley-errors-messages-disabled onsubmit="return false">
                ...
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="isactive">Material Sub 1 *</label>
                        <select class="form-control" name="SUBGROUP1" id="SUBGROUP1" required>
                            <option value="" selected disabled>Choose Sub 1</option>
                            <?php
                            foreach ($DtMaterialSub as $values) {
                    ...
                    <div class="form-group col-md-6">
                        <label for="isactive">Material Sub 2 *</label>
                        <select class="form-control" name="SUBGROUP2" id="SUBGROUP2" required>
                            <option value="" selected disabled>Choose Sub 2</option>
                            <?php
                            foreach ($DtMaterialSub as $values) {
                                echo '<option value=' . $values->FCCODE . '>' . $values->FCNAME . '</option>';
                            }
...
<script>
    ...
    function SetDataKosong() {
        $('.panel-title').text('Add Data Material Group');
        ID = 0;
        $('#FCCODE').val('');
        $('#FCNAME').val('');
        $('#SUBGROUP1').val('');
    ...
    function SetData(data) {
        $('.panel-title').text('Edit Data Material Group');
        ID = data.ID;
//        $('#FCCODE').attr('readonly', true);
        $('#FCCODE').val(data.FCCODE);
        $('#FCNAME').val(data.FCNAME);
        $('#SUBGROUP1').val(data.SUBGROUP1);
        $('#SUBGROUP2').val(data.SUBGROUP2);
