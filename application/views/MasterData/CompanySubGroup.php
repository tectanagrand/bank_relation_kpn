<ol class="breadcrumb pull-right">
    ...
    <li class="breadcrumb-item active">Company SubGroup</li>
</ol>
<h1 class="page-header">Company SubGroup</h1>
<div class="panel panel-success">
    ...
    <div class="panel-heading">
        ...
        <h4 class="panel-title">Company SubGroup</h4>
    </div>
    <div class="panel-body">
        <?php if (empty($_GET)) { ?>
            ...
            <div class="row m-0 table-responsive">
                <table id="DtCompanySubGroup" class="table table-striped table-bordered table-hover dataTable no-footer dtr-inline" role="grid" width="100%" aria-describedby="DtCompanySubGroup_info" style="width: 100%;">
                    <thead>
                        <tr role="row">
                            <th class="text-center sorting_asc" style="width: 30px;">No</th>
                            <th class="text-center sorting">Comp. SubGroup Code</th>
                            <th class="text-center sorting">Comp. SubGroup Name</th>
            ...
            <form id="FAddEditForm" data-parsley-validate="true" data-parsley-errors-messages-disabled onsubmit="return false">
                <div class="row">
                    <div class="form-group col-md-6">
                        <label for="fccode">Comp. SubGroup Code *</label>
                        <input type="text" class="form-control" name="FCCODE" id="FCCODE" placeholder="Comp. SubGroup Code" required>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="fcname">Comp. SubGroup Name *</label>
                        <input type="text" class="form-control" name="FCNAME" id="FCNAME" placeholder="Comp. SubGroup Name" required>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group col-md-6">
...
<script>
    ...
    $(document).ready(function () {
        if (getUrlParameter('type') == "edit" || getUrlParameter('type') == "add") {
            ...
            if (getUrlParameter('type') == "add") {
                ...
                var data = <?php echo json_encode($DtCompanySubGroup); ?>;
                SetData(data);
            }
        } else {
            if (!$.fn.DataTable.isDataTable('#DtCompanySubGroup')) {
                $('#DtCompanySubGroup').DataTable({
                    "processing": true,
                    "ajax": {
                        "url": "<?php echo site_url('ICompanySubGroup/ShowData') ?>",
                ...
                $('#DtCompanySubGroup thead th').addClass('text-center');
                table = $('#DtCompanySubGroup').DataTable();
                ...
                table.on('click', '.delete', function () {
                    ...
                    if (confirm('Are you sure delete this data "' + data.FCNAME + '" ?')) {
                        $.ajax({
                            dataType: "JSON",
                            type: "POST",
                            url: "<?php echo site_url('ICompanySubGroup/Delete'); ?>",
                            data: {
                                FCCODE: data.FCCODE,
                                USERNAME: USERNAME
                            },
                ...
                $("#DtCompanySubGroup_filter").remove();
                $("#search").on({
                    'keyup': function () {
    ...
    function SetDataKosong() {
        $('.panel-title').text('Add Data Region');
        $('#FCCODE').val('');
        $('#FCNAME').val('');
        $('#DESCRIPTION').val('');
        // $('#ISACTIVE').val('TRUE');
    ...
    function SetData(data) {
        $('.panel-title').text('Edit Data Region');
    ...
    var Save = function () {
        if ($('#FAddEditForm').parsley().validate()) {
            ...
            $.ajax({
                dataType: "JSON",
                type: "POST",
                url: "<?php echo site_url('ICompanySubGroup/Save'); ?>",
                data: {
                    FCCODE: $('#FCCODE').val(),
                    FCNAME: $('#FCNAME').val(),
