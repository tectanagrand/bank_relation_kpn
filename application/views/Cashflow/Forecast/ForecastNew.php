...
<h1 class="page-header">Forecast New</h1>
<div class="panel panel-success">
    <div class="panel-heading">
        <div class="panel-heading-btn">
            <a href="javascript:;" class="btn btn-xs btn-icon btn-circle btn-default" data-click="panel-expand"><i class="fa fa-expand"></i></a>
        </div>
        <h4 class="panel-title">Forecast New</h4>
    </div>
                ...
                <div class="form-row">
                    <div class="col my-auto" style="width: 150px">
                        <?php if ($DtUser2->USERACCESS != '100003') { ?>
                            ...
                            <button id="btnAddAll" type="button" class="btn btn-primary btn-sm" style="display:none;"><i class="fa fa-plus"></i><span> Forecast All</span></button></br></br>
            ...
            <div class="modal-body">
                <div class="table-responsive">
                    <table id="DtOutstandingNoInv" Document class="table table-striped table-bordered" cellspacing="0" width="100%" aria-describedby="DtUploadDetail_info">
                        <thead>
                            <tr>
                                <th class="text-center"><input type="checkbox" id="pilNoinv"></th>
                                <th class="text-center">No</th>
                                <th class="text-center">Company</th>
                                <th class="text-center">Business Unit</th>
                                <th class="text-center">Doc Type</th>
...
<script>
    ...
    var cekFC      = "<?php echo $SESSION->status_fc; ?>";
    var YEAR = "",
        MONTH = "",
        timeOutonKeyup = null;
    var PSTATUS = '0',
        ...
        POUTSTAND = "",
        AMOUNTUSE = 0,
        idx, WAMOUNT, LAMOUNT, WPRIORITY, oldVal;
    ...
    var EditAmount = function() {
        if ($('#FEditAmount').parsley().validate()) {
            // var getOldVal  = formatDesimal(oldVal);
            ...
            if (parseFloat(DtForecast[idx].AMOUNTOUTSTANDING) < (parseFloat(AMOUNTUSE) + parseFloat(AMOUNTWEEK))) {
                ...
                if(parseFloat(oldVal) != parseFloat(AMOUNTWEEK)){
                    setFC(1);
                }else{
                    setFC(0);
                }
        ...
        'change': function() {
            ...
            if (YEAR == null || YEAR == '' || YEAR == undefined) {
                ...
                $.ajax({
                    dataType: "JSON",
                    type: "POST",
                    url: "<?php echo site_url('Forecast/ShowDtForecastNew'); ?>",
                    data: {
                        YEAR: YEAR,
                        MONTH: MONTH,
                ...
                $.ajax({
                    ...
                    success: function(response, textStatus, jqXHR) {
                        ...
                        if (response.status == 200) {
                            ...
                            //     dataType: "JSON",
                            //     type: "POST",
                            //     url: "<?php echo site_url('Forecast/CekValidationNegatif'); ?>",
                            //     data: {
                            //         YEAR: YEAR,
                            //         MONTH: MONTH,
                            //         DEPARTMENT: $("#DEPARTMENT").val(),
        ...
        if ($('#PERIOD').val() == null || $('#PERIOD').val() == '' || $('#PERIOD').val() == undefined) {
            ...
            if (!$.fn.DataTable.isDataTable('#DtOutstanding')) {
                $('#DtOutstanding').DataTable({
                    ...
                    dom: 'Bfrtip',
                    "ajax": {
                        "url": "<?php echo site_url('Forecast/DtOutstandingNew') ?>",
                        "type": "POST",
                        "datatype": "JSON",
                        "data": function(d) {
                            d.MONTH = MONTH;
        ...
        if ($('#PERIOD').val() == null || $('#PERIOD').val() == '' || $('#PERIOD').val() == undefined) {
            ...
            if (!$.fn.DataTable.isDataTable('#DtOutstandingNoInv')) {
                $('#DtOutstandingNoInv').DataTable({
                    ...
                    dom: 'Bfrtip',
                    "ajax": {
                        "url": "<?php echo site_url('Forecast/DtOutstandingNoInvNew') ?>",
                        "type": "POST",
                        "datatype": "JSON",
                        "data": function(d) {
                            d.MONTH = MONTH;
    ...
    var setFC = function(cekFC){
        $.ajax({
            dataType: "JSON",
            type: "POST",
            url: "<?php echo site_url('Forecast/set_session'); ?>",
            data: {
                cekFC: cekFC,
                USERNAME: USERNAME
            },
    ...
    $("#MOutstandingNoInv button[type='submit']").on('click', function() {
        ...
        $.each(table3.data(), function(index, value) {
            if (value.ID == undefined || value.ID == null || value.ID == '') {} else {
                 if (value.FLAG == 1) {
                    $.ajax({
                        url: "<?php echo site_url('Forecast/AddDummyInvNew') ?>",
                        type: "POST",
                        data: {
                            ID: value.ID,
                            YEAR: YEAR,
                            MONTH: MONTH,
                            AMOUNT: value.AMOUNTOUTSTANDING
                        },
                        dataType: "JSON",
    ...
    // End Region                                                  
    $('#pilNoinv').on('change', function() {
        if (this.checked) {
            $('#DtOutstandingNoInv .pils').prop("checked", true);
        } else {
            $('#DtOutstandingNoInv .pils').prop("checked", false);
        }
        $('#DtOutstandingNoInv .pils').change();
    });
    ...
    var Save = function(ACTION) {
        if (YEAR == null || YEAR == '' || YEAR == undefined) {
            ...
            $.ajax({
                dataType: "JSON",
                type: "POST",
                url: "<?php echo site_url('Forecast/SaveForecastTemp'); ?>",
                data: {
                    YEAR: YEAR,
                    MONTH: MONTH,
                ...
                success: function(response, textStatus, jqXHR) {
                    ...
                    if (response.status == 200) {
                        if(response.result.data != 'Data has been Successfully Saved !!'){
                            alert(response.result.data);
                            $('#PERIOD').val('');
                            POUTSTAND = "";
                            YEAR = '';
                        ...
                        if(ACTION == "2" || ACTION == 2 || ACTION == "1" || ACTION == 1){
    ...
    var Submit = function() {
        ...
        if($('#btnEdit').is(":hidden")){
            ...
            alert('Click Edit Forecast First!');
        }
    };
