    ...
    <div class="panel-body">
                ...
                <div class="form-row">
                    <div class="col my-auto" style="width: 150px">
                        <?php if ($DtUser2->USERACCESS != '100003') { ?>
                            ...
                            <button id="btnEdit" type="button" class="btn btn-info btn-sm"><i class="fa fa-edit"></i><span> Edit Forecast</span></button>
                        <?php } ?>
        ...
        <div class="row m-0 table-responsive" id="dtf">
            <table id="DtForecast" class="table table-striped table-bordered " cellspacing="0" width="100%" aria-describedby="DtUpload_info">
                ...
                <tfoot>
                    <tr role="row">
                        <th class="text-right align-middle" colspan="11" rowspan="2">Total Cash </th>
...
<?php } ?>
    ...
    var FCLOCK = "", FCSTATUS = "";
    var LOCKIN = 0, LOCKOUT = 0, STATUSIN = 0, STATUSOUT = 0, ISEDIT = 0, KEYSAVE = "";
    ...
    if (!$.fn.DataTable.isDataTable('#DtForecast')) {
        $('#DtForecast').DataTable({
            ...
            "preDrawCallback": function (settings) {
                ...
                scrollLeft = $('div.dataTables_scrollBody').scrollLeft();
            },
            "drawCallback": function (settings) {
                $('div.dataTables_scrollBody').scrollTop(pageScrollPos);
                $('div.dataTables_scrollBody').scrollLeft(scrollLeft);
            },
            "columns": [
                {
                    "data": "CASHFLOWTYPE"
...
//        alert('test');
        ...
        table.rows.add(DtForecast);
        table.draw();
        // var scrollPos = $("table#DtForecast .dataTables_scrollBody").scrollTop();
        // $('#DtForecast').DataTable().rows().invalidate().draw(false);
        // $("table#DtForecast .dataTables_scrollBody").scrollTop(scrollPos);
    };
...
//    Add Data Forecast
    ...
    var AddNoInv = function () {
        if ($('#PERIOD').val() == null || $('#PERIOD').val() == '' || $('#PERIOD').val() == undefined) {
            ...
            if (!$.fn.DataTable.isDataTable('#DtOutstandingNoInv')) {
                ...
                tablex = $('#DtOutstandingNoInv').DataTable();
                tablex.on('change', '.pils', function () {
                    $tr = $(this).closest('tr');
    ...
    $("#MOutstandingNoInv button[type='submit']").on('click', function () {
        ...
        $(this).prop('disabled', true);
        $('#loader').addClass('show');
        $.each(tablex.data(), function (index, value) {
            if (value.FLAG == 1){
                $.ajax({
...
//    Export Data
    ...
    var Export = function (type) {
        if ($('#FExport').parsley().validate()) {
            var url = "<?php echo site_url('Process/ForecastExport'); ?>?type=PARAM1&DEPARTMENT=PARAM2&CASHFLOWTYPE=PARAM3&MONTH=PARAM4&YEAR=PARAM5&USERNAME=PARAM6";
