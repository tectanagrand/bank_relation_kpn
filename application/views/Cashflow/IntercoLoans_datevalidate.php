    ...
    <div class="panel-body">
        <form id="FAddEditForm" data-parsley-validate="true" data-parsley-errors-messages-disabled onsubmit="return false">
            <div class="row">
                ...
                <div class="col-md-2">
                    <input type="text" id="searchInvoice" class="form-control mb-2" placeholder="Cari.." >
                </div>
            </div>
                ...
                <form id="formAddPayment" data-toggle="validator" role="form">
                    <div class="row">
                        <div class="form-group col-md-5">
                            <label for="PAY_BANK">Company Sender*</label>
                        ...
                        <div class="form-group col-md-5">
                            <label for="PAY_BANK">Bank Receiver*</label>
						...
						<div class="form-group col-md-4">
                            <label for="SOURCEAMOUNT">Bank Sender Amount *</label>
...
<script>
    ...
    $(document).ready(function () {
        ...
        var date = new Date();
        var mDate = new Date(date.getFullYear(), date.getMonth(), date.getDate());
        ...
        $(document).on('change', '#DATE_PAID2', function () { 
            ...
            $('#PAY_PAID_AMOUNT').prop("disabled", false);
			$('#SOURCEAMOUNT').prop("disabled", false);
			$('#PAY_RATE').prop("disabled", false);
            $('#PAY_BANK').prop("disabled", false);
			$('#PAY_BANKTARGET').prop("disabled", false);
		...
		$('#SOURCEAMOUNT').on('input', function() {
			...
			var pay_rate = Number($('#PAY_RATE').val().replace(/[^0-9.-]+/g,""));
			var source_amount = Number($(this).val().replace(/[^0-9.-]+/g,""));
			var fixamount = 0;
			...
			$.ajax({
				...
				success: function(data)
				{
					...
					if (data.CURRSOURCE === 'USD' && data.CURRTARGET === 'IDR')  {
						formatCurrency($('#PAY_PAID_AMOUNT').val(pay_rate * source_amount), 'blur')					
					}else if (data.CURRSOURCE === 'IDR' && data.CURRTARGET === 'USD')  {
    ...
    function LoadDataTable() {
		...
		deferRender: true,
            ...
            table2.on('click', '.edit', function() {
                ...
                $('#PAY_VOUCHER').attr('disabled',true);
				...
				$('#PAY_BANKTARGET').prop("disabled", true);
				$('#COMPANY').prop("disabled", true);
                $('#COMPANYTARGET').prop("disabled", false);
