...
<ol class="breadcrumb pull-right">
    ...
    <li class="breadcrumb-item active">Upload SAP Hana PO 5</li>
</ol>
<h1 class="page-header">Upload SAP Hana PO 5</h1>
<div class="panel panel-success">
    <div class="panel-heading">
        <div class="panel-heading-btn">
            <a href="javascript:;" class="btn btn-xs btn-icon btn-circle btn-default" data-click="panel-expand"><i class="fa fa-expand"></i></a>
        </div>
        <h4 class="panel-title">Upload SAP Hana PO 5</h4>
    </div>
        ...
        <div class="note note-yellow m-b-10">
            ...
            <div class="note-content">
                ...
                <ul class="m-b-5 p-l-20">
                    <li>The maximum file size for uploads in this transaction is <strong> 1 MB</strong> (default file size is unlimited).</li>
...
<script>
    ...
    if (!$.fn.DataTable.isDataTable('#DtUpload')) {
        $('#DtUpload').DataTable({
            ...
            "columns": [{
                ...
                // {
                //     "data": null,//BUSINESSUNITCODE
                //     "className": "text-center",
                //     render: function(data, type, row, meta) {
                //         var html = '';
                //         if (data.BUSINESSUNITCODE == 'BUSINESSUNIT NOT FOUND') {
                //             html += '<span class="badge badge-pill badge-danger" title="' + data.BUSINESSUNITCODE + '">' + data.BUSINESSUNITCODE + '</span>';
                //         } else {
                //             html += '<span class="badge badge-pill badge-success">BUSINESSUNIT FOUND</span>';
    ...
    function filesChange(elm) {
        if ($('#DEPARTMENT').val() == '' || $('#DEPARTMENT').val() == null || $('#DEPARTMENT').val() == undefined) {
            alert('Please, Choose Department Firts!!!');
            files = '';
            $('.upload-file').val('');
        } else {
                ...
                if (fileInput.get(0).files.length) {
                    ...
                    if (fileSize > maxSize) {
                        ...
                        $.ajax({
                            dataType: "JSON",
                            type: 'POST',
                            url: "<?php echo site_url('Upload/UploadPO5Hana'); ?>",
                            data: fd,
                            processData: false,
                            contentType: false,
    ...
    var Save = function() {
        if (STATUS == false) {
            ...
            $.ajax({
                dataType: "JSON",
                type: "POST",
                url: "<?php echo site_url('Upload/Save_POHana'); ?>",
                data: {
