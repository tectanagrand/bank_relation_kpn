...
<script>
    ...
    $('#DATEFROM').on({
        'change': function () {
            sMonth = ListBulan.indexOf(this.value.substr(0, 3)) + 1;
            sYear = this.value.substr(4, 4);
            DataReload();
        }
    });
    ...
    if (!$.fn.DataTable.isDataTable('#DtPayment')) {
        ...
        table.on('click', '.edit', function () {
            ...
            $('#DATERELEASE').datepicker({
                    "autoclose": true,
                    "format": "mm/dd/yyyy",
                    "startDate": moment(data.DATERELEASE).format('MM/DD/YYYY'),
                    // "setDate": data.DATERELEASE
            });              
