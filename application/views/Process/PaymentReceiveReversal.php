...
<ol class="breadcrumb pull-right">
    ...
    <li class="breadcrumb-item active">Payment And Receive Reversal</li>
</ol>
<!-- <h1 class="page-header">Payment And Receive Reversal</h1> -->
<ul class="nav nav-tabs">
    <li class="nav-item">
        <a href="#paymentreceive" data-toggle="tab" class="nav-link active">
            <span class="d-sm-none">Tab 1</span>
            <span class="d-sm-block d-none">Payment and Receive Reversal</span>
        </a>
    </li>
   <!--  <li class="nav-item">
    ...
    <div class="tab-pane fade active show" id="paymentreceive">
        <div class="panel panel-success">
            <div class="panel-heading">
                ...
                <h4 class="panel-title">Payment And Receive Reversal</h4>
            </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="PAYMENTTYPE" id="PAYMENTTYPE" required>
                                                ...
                                                <option value="0">Receive Reversal For SO</option>
                                                <option value="1">Payment Reversal For PO</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                    ...
                                    <table id="DtPayment1" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                        <thead>
                                            <tr role="row">
                                                ...
                                                <th class="text-center align-middle" rowspan="2">Due Date</th>
                                                <th class="text-center align-middle" rowspan="2">Currency</th>
                                                <th class="text-center align-middle" rowspan="2">Ref Voucher</th>
                                                <th class="text-center align-middle" rowspan="2">Ref Amount</th>
                                                <th class="text-center align-middle" rowspan="2">Ref Amount Bank</th>
                                                <th class="text-center" colspan="4">Amount</th>
                                                <th class="text-center align-middle" rowspan="2"></th>
                                            </tr>
                                            <tr>
...
var LoadData = function() {
    ...
    if (!$.fn.DataTable.isDataTable("#DtPayment1")) {
        table3 = $("#DtPayment1").DataTable({
            ...
            {
                "data": "DOCREF",
                "className": "text-center dpDOCREF"
            },
            {
                "data": "VENDORNAME"
            },
            ...
            },
            {
                ...
                "className": "text-center ",
                render: function(data, type, row, meta) {
                    var html = "<select class='form-control refVoucher'><option>--</option></select>";
                    return html;
                }
            },
            {
                "data": null,
                "className": "text-center refAmount",
                render: function(data, type, row, meta) {
                    var html = "";
                    return html;
                }
            ...
            {
                "data": null,
                "className": "text-center refAmountBank",
                render: function(data, type, row, meta) {
                    var html = "";
                    return html;
                }
...
var EditAPaid = function() {
    ...
    // $(document).on('load', '.refVoucher', function () {            
    //     $.ajax({
    //             url : "<?php echo site_url('Payment/getRefVoucher');?>",
    //             method : "POST",
    //             data : {
    //                 // COMPANY: $('#COMPANYCODE').val(),
    //                 DOCNUMBER: $('.dpDOCREF').val()
    //             },
    //             async : true,
    //             dataType : 'json',
    //             success: function(data){
    ...
    //                         var i;
    //                         for(i=0; i<data.result.data.length; i++){
    //                             listSub += '<option value='+data.result.data[i].VOUCHERNO+'>'+data.result.data[i].VOUCHERNO+'</option>';
    //                         }
    //                         $('.refVoucher').html(listSub);
    //                         $('#loader').removeClass('show');
...
function showDtOs(){
    if (!$.fn.DataTable.isDataTable('#DtOS')) {
                table2 = $('#DtOS').DataTable(
                {
                    ...
                    "processing": true,
                            "ajax": {
                                "url": "<?php echo site_url('Payment/DtOsPayment1Reversal') ?>",
                                "type": "POST",
                                "datatype": "JSON",
                                "language": {
            ...
            if (value.FLAG == 1) {
                ...
                $.ajax({
                        ...
                        success: function(data){
                            ...
                            var listSub = '<option selected disabled>Select</option>';
                            var refAmt  = '';
                            var refAmtBank  = '';
                            var i;
                            for(i=0; i<data.result.data.length; i++){
                                listSub += '<option value='+data.result.data[i].VOUCHERNO+' data-amount='+data.result.data[i].AMOUNT+' data-amountbank='+data.result.data[i].AMOUNTBANK+'>'+data.result.data[i].VOUCHERNO+'</option>';
                                // refAmt  += data.result.data[i].AMOUNT;
                            }
                            $('.refVoucher').html(listSub);
                            $(".refVoucher").on({
                                'change': function() {

                                    $('#DtPayment1 td.refAmount').text(fCurrency($('.refVoucher').find(':selected').attr('data-amount')));
                                    $('#DtPayment1 td.refAmountBank').text(fCurrency($('.refVoucher').find(':selected').attr('data-amountbank')));
                                    $('#DtPayment1 a.AMOUNTPAID').text(formatMoney(intVal($('.refVoucher').find(':selected').attr('data-amount')) * -1));
                                }
                            });
                            $('#loader').removeClass('show');
...
var DisablePC = function() {
    ...
    var Save = function() {
        ...
        if ($('#FAddEdit').parsley().validate()) {
            if (DtPaid.length <= 0) {
            ...
            else if($('#DtPayment1 .refVoucher').val() == '' || $('#DtPayment1 .refVoucher').val() == null){
                alert("Can't empty");
            } else {
                $("#loader").addClass('show');
                ...
                $.ajax({
                    dataType: "JSON",
                    type: "POST",
                    url: "<?php echo site_url('Payment/SavePaymentReversal'); ?>",
                    data: {
                        ...
                        AMOUNTPAID: $("td a.AMOUNTPAID").text(),
                        REF_VOUCHER: $('.refVoucher').find(':selected').val(),
                        DtPaid: DtPaid,
                        USERNAME: USERNAME
                    },
                    success: function(response, textStatus, jqXHR) {
