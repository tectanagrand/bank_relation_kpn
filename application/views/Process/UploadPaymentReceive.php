        ...
        <div class="tab-pane fade active show" id="import">
            <div class="panel panel-success">
                    ...
                    <div class="row mb-2 fileupload-buttonbar">
                        <div class="col-md-12">
                            <select class="form-control-sm" name="EXTSYSTEM" id="EXTSYSTEM" required="">
                                ...
                                <option value="IPLAS">IPLAS</option><option value="SAP">SAP</option><option value="SAPHANA">SAPHANA</option><option value="TIPTOP">TIPTOP</option>                </select>
                            <select class="form-control-sm" name="DOCTYPE" id="DOCTYPE" required="">
                                <option value="" selected="" disabled="">Choose Doc Type</option>
                                <option value="INV">INV</option>
                                <option value="INV_AP_SPC">INV AP SPC</option>
                                <option value="PAYMENT_OTHERS">PAYMENT OTHERS</option>
                                <!-- <option value="PINDAH_BUKU">PAYMENT PINDAH BUKU</option> -->
                            </select>
                            <select class="form-control-sm" name="CASHFLOWTYPE_2" id="CASHFLOWTYPE_2">
                ...
                <div class="row m-0 table-responsive">
                    <table id="DtUpload" class="table table-striped table-bordered d-none" cellspacing="0" width="100%" aria-describedby="DtUpload_info">
                        <thead>
                            <tr role="row">
                                ...
                                <th class="text-center sorting">Amount Convert</th>
                    ...
                    <table id="DtUploadOthers" class="table table-striped table-bordered d-none" cellspacing="0" width="100%" aria-describedby="DtUploadOthers_info">
                        <thead>
                            <tr role="row">
                                <th class="text-center sorting_asc" aria-sort="ascending" style="width: 30px;">No</th>
                                <th class="text-center">Status</th>
...
<script>
    $(document).ready(function(e){
        $('#DtUpload_wrapper').hide();
        $('#DtUploadOthers_wrapper').hide();
    });
...
<script>
    ...
    if (!$.fn.DataTable.isDataTable('#DtUploadOthers')) {
        $('#DtUploadOthers').DataTable({
            "aaData": DtUploadOthers,
            "columns": [
            {
                "data": null,
                "className": "text-center",
        ...
        tbl_uploadOthers = $('#DtUploadOthers').DataTable();
    }

    $('#DOCTYPE').on({
        'change': function() {
            if(this.value == "PAYMENT_OTHERS"){
                $('#DtUpload_wrapper').hide();
                $('#DtUploadOthers_wrapper').show();
            }
            else{
                $('#DtUpload_wrapper').show();
                $('#DtUploadOthers_wrapper').hide();
            }
        }
    });
    ...
    function filesChange(elm) {
        ...
        var CASHFLOWTYPE_2      = $('#CASHFLOWTYPE_2').val();
        ...
        if (CASHFLOWTYPE_2 == '' || CASHFLOWTYPE_2 == null || CASHFLOWTYPE_2 == undefined) {
        ...
        if (EXTSYSTEM == '' || EXTSYSTEM == null || EXTSYSTEM == undefined) {
            alert('Please, Choose Extsys First!!!');
            files = '';
            $('.upload-file').val('');
        }
        else {
                ...
                if (fileInput.get(0).files.length) {
                    ...
                    if (fileSize > maxSize) {
                        ...
                        if(DOCTYPE == "PAYMENT_OTHERS"){
                            ...
                            fd.append("CASHFLOWTYPE", CASHFLOWTYPE_2);
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    ...
                                    if (response.status == 200) {
                                        STATUS = true;
                                        DtUploadOthers = response.result.data;
                                        $('#DtUploadOthers').removeClass('d-none');
                                        tbl_uploadOthers.clear();
                                        tbl_uploadOthers.rows.add(DtUploadOthers);
                                        tbl_uploadOthers.draw();
            ...
            if (DtUploadOthers.length <= 0) {
                ...
                if (STATUS == false) {
                    ...
                    $.ajax({
                        ...
                        data: {
                            DATA: JSON.stringify(DtUploadOthers),
                            FILENAME: FILENAME,
                            DOCTYPE: DOCTYPE,
                            USERNAME: USERNAME
                        },
    ...
    //             dataType: "JSON",
    //             type: "POST",
    //             url: "<?php echo site_url('Upload/clearPaymentReceive'); ?>",
    //             data: {
    //                 DOCTYPE: $('#DOCTYPE').val()
    //             },
    //             success: function (response) {
